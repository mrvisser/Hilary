...
ActivityEmitter.on(ActivityConstants.events.DELIVERED_ACTIVITIES, function(deliveredActivities) {
    ...
    _getEmailRecipientResources(emailRecipientIds, function(err, recipients) {
        ...
        _createEmailBucketIdForRecipients(recipientsToQueue, function(err, emailBucketIds) {
            if (err) {
                scheduledEmailsCounter.decr();
                return log().error({'err': err, 'recipientIdsToQueue': _.pluck(recipientsToQueue, 'id')}, 'Unable to create email bucket ids of the users who need to receive mail');
            }

            var emailBuckets = {};
            _.chain(recipientsToQueue)
                .zip(emailBucketIds)
                .each(function(recipientToQueueWithEmailBucketId) {
                    var recipient = recipientToQueueWithEmailBucketId[0];
                    var emailBucketId = recipientToQueueWithEmailBucketId[1];
                    emailBuckets[emailBucketId] = emailBuckets[emailBucketId] || [];
                    emailBuckets[emailBucketId].push(recipient.id);
                })
                .value();
...
PrincipalsEmitter.on(PrincipalsConstants.events.UPDATED_USER, function(ctx, newUser, oldUser) {
    ...
    // Generate bucket ids for the users
    _createEmailBucketIdForRecipients([oldUser, newUser], function(err, emailBucketIds) {
        if (err) {
            return callback(err);
        }

        var oldBucketId = emailBucketIds[0];
        var newBucketId = emailBucketIds[1];
        ...
        ActivityDAO.unqueueUsersForEmail(oldBucketId, [oldUser.id], function(err) {
            ...
            // Queue the user for their new email preference. If they have no
            // pending emails, they will be ignored during the collection cycle
            var emailBucket = {};
...
var _mail = function(recipient, activities, callback) {
    ...
    ActivityTransformer.transformActivities(ctx, aggregatedActivities, ActivityConstants.transformerTypes.ACTIVITYSTREAMS, function(err) {
        ...
        // Get the skin data for the email template
        UIAPI.getTenantSkinVariables(tenant.alias, function(err, tenantSkinVariables) {
            if (err) {
                return callback(err);
            }

            TenantConfig.getValue(recipient.tenant.alias, 'timezone', 'timezone', function(err, timezone) {
                if (err) {
                    return callback(err);
                }
                ...
                var data = {
                    ...
                    'skinVariables': tenantSkinVariables,
                    'timezone': timezone
                };
 ...
 *
 * @param  {Resource[]}     recipients                  The recipient profiles for which to generate email bucket ids
 * @param  {Function}       callback                    Standard callback function
 * @param  {Object}         callback.err                An error that occurred, if any
 * @param  {String[]}       callback.emailBucketIds     The email bucket ids, in the same order as the provided recipients
 * @api private
 */
var _createEmailBucketIdForRecipients = function(recipients, callback, _recipients, _emailBucketIds) {
    _recipients = _recipients || recipients.slice();
    _emailBucketIds = _emailBucketIds || [];
    if (_.isEmpty(_recipients)) {
        return callback(null, _emailBucketIds);
    }

    var recipient = _recipients.shift();
    TenantConfig.getValue(recipient.tenant.alias, 'timezone', 'timezone', function(err, timezone) {
        if (err) {
            return callback(err);
        }
        ...
        _emailBucketIds.push(_createEmailBucketId(bucketNumber, recipient.emailPreference, dayOfWeek, hour));
        return _createEmailBucketIdForRecipients(recipients, callback, _recipients, _emailBucketIds);
    });
};
