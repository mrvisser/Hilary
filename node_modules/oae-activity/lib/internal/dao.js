...
var saveAggregates = module.exports.saveAggregates = function(routeAggregateKeys, streamUpdates, callback) {
    ...
    routes.forEach(function(route) {
        routeAggregateKeys[route].forEach(function(aggregateKey) {
            var actor = streamUpdates[route].actor;
            var object = streamUpdates[route].object;
            var target = streamUpdates[route].target;

            // Add the aggregate status
            queries.push({
                'query': 'UPDATE ActivityAggregationStatus SET lastModified = ? WHERE aggregateKey = ?',
                'parameters': [now, aggregateKey]
            });
