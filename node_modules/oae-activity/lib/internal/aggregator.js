                ...
                ActivityDAO.getAggregatedEntities(_.keys(activeAggregates), function(err, fetchedEntities) {
                    ...
                    ActivityDAO.saveAggregatedEntities(allAggregates, function(err) {
                        ...
                        _.keys(aggregatesToDeliver).forEach(function(aggregateKey) {
                            ...
                            if (actor && actor[ActivityConstants.properties.OAE_COLLECTION] && _.compact(actor[ActivityConstants.properties.OAE_COLLECTION]).length !== actor[ActivityConstants.properties.OAE_COLLECTION].length) {
                                log().warn({'actor': actor}, 'Actor had null items in collection');
                            }

                            if (object && object[ActivityConstants.properties.OAE_COLLECTION] && _.compact(object[ActivityConstants.properties.OAE_COLLECTION]).length !== object[ActivityConstants.properties.OAE_COLLECTION].length) {
                                log().warn({'object': object}, 'Object had null items in collection');
                            }

                            if (target && target[ActivityConstants.properties.OAE_COLLECTION] && _.compact(target[ActivityConstants.properties.OAE_COLLECTION]).length !== target[ActivityConstants.properties.OAE_COLLECTION].length) {
                                log().warn({'target': target}, 'Target had null items in collection');
                            }
...
var _createActivityEntity = function(entities) {
    if (!entities) {
        ...
        return entities[0];
    } else if (_.compact(entities).length !== entities.length) {
        log().warn({'entities': entities}, 'Entities array had null item(s)');
    }
