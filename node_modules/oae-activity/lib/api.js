...
var AGGREGATE_EXPIRY_IN_SECONDS = 60 * 15;
...
var _handleActivityTask = function(activitySeed, callback) {
    ...
    _produceAllRoutes(activitySeed, function(err, actorRoutes, objectRoutes, targetRoutes) {
        ...
        _produceAllEntities(activitySeed, function(err, actor, actorPropagation, object, objectPropagation, target, targetPropagation) {
            ...
            // Push the activity to the aggregate routes, and remove the ones that don't need to be routed
            ...
            ActivityDAO.getAggregateLastModifiedByKeys(_.flatten(_.values(routeAggregateKeys)), function(err, activeAggregates) {
                if (err) {
                    return callback(err);
                }
                ...
                ActivityDAO.saveAggregates(routeAggregateKeys, activityStreamUpdates, function(err) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    ActivityDAO.saveActivities(activityStreamUpdates, callback);
                });
            });
        });
    });
