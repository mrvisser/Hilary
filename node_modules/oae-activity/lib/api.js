...
var activityRouters = {};
var aggregationTimerId = null;
var boundWorker = false;

var RESOURCE_KEY_EMPTY = '__null__';
var AGGREGATE_EXPIRY_IN_MS = 15 * 60 * 1000; // 15 Minutes
...
var _handleActivityTask = function(activitySeed, callback) {
    ...
    var activityKey = published + ':' + ShortId.generate();

    // Produce the routes for the actor, object and target resources
    _produceAllRoutes(activitySeed, function(err, actorRoutes, objectRoutes, targetRoutes) {
        ...
        _produceAllEntities(activitySeed, function(err, actor, actorPropagation, object, objectPropagation, target, targetPropagation) {
            ...
            for (var i = 0; i < actorRoutes.length; i++) {
                ...
                activityStreamUpdates[actorRoute] = activityStreamUpdates[actorRoute] || new Activity(activityType, activityId, published, verb);
            ...
            for (var i = 0; i < objectRoutes.length; i++) {
                ...
                activityStreamUpdates[objectRoute] = activityStreamUpdates[objectRoute] || new Activity(activityType, activityId, published, verb);
            ...
            for (var i = 0; i < targetRoutes.length; i++) {
                ...
                activityStreamUpdates[targetRoute] = activityStreamUpdates[targetRoute] || new Activity(activityType, activityId, published, verb);
            ...
            }, 'Built activity updates.');

            // Push the activity to the aggregate routes, and remove the ones that don't need to be routed (i.e., they have live aggregations)
            var allRoutes = _.uniq(_.union(actorRoutes, objectRoutes, targetRoutes));
            var routeAggregateKeys = _createAggregateKeys(allRoutes, activitySeed);
            var allAggregateKeys = _.flatten(_.values(routeAggregateKeys));
            ActivityDAO.getAggregateLastModifiedByKeys(allAggregateKeys, function(err, activeAggregates) {
                if (err) {
                    return callback(err);
                }

                // We just need to know they're active, not what their actual lastModified time is
                var activeAggregateKeys = _.keys(activeAggregates);

                // We need to know all the new aggregates so that we can record their 'last activity key' when they are routed
                var newAggregateKeys = _.intersect(activeAggregateKeys, allAggregateKeys);

                // Now that we have the active aggregates, create/update all the new ones
                ActivityDAO.saveAggregates(routeAggregateKeys, activityKey, activityStreamUpdates, function(err) {
                    if (err) {
                        return callback(err);
                    }

                    // Ensure the active aggregates are marked as such. It means that the aggregation interval job will pick them up and aggregate them
                    ActivityDAO.setAggregatesActive(activeAggregateKeys, function(err) {
                        if (err) {
                            return callback(err);
                        }

                        // Suppress the activity stream updates that already have active aggregates
                        var suppressedRoutes = [];
                        activeAggregateKeys.forEach(function(aggregateKey) {
                            var route = aggregateKey.split('#')[1];
                            suppressedRoutes.push(route);
                            delete activityStreamUpdates[route];
                        });

                        if (activeAggregateKeys.length > 0) {
                            log().trace({
                                'activitySeed': activitySeed,
                                'routes': suppressedRoutes,
                                'aggregateKeys': activeAggregateKeys
                            }, 'Suppressing activity delivery to routes due to active aggregates.');
                        }

                        // Store the routed activities in the DB
                        ActivityDAO.saveActivities(activityStreamUpdates, function(err) {
                            if (err) {
                                return callback(err);
                            }

                            ActivityDAO.setLastActivityKey(newAggregateKeys, activityKey, callback);
                        });
                    });
                });
            });
...
var _createAggregateKeys = function(routes, activitySeed) {
    var keys = {};
    var groupBy = (activityTypes[activitySeed.activityType]) ? activityTypes[activitySeed.activityType].groupBy : [];

    for (var groupByIndex = 0; groupByIndex < groupBy.length; groupByIndex++) {
        var pivot = groupBy[groupByIndex];
        for (var routeIndex = 0; routeIndex < routes.length; routeIndex++) {
            ...
            // Build the potential aggregate key and push it onto the route
            var actorKey = (pivot.actor) ? _createResourceKey(activitySeed.actorResource) : '';
            var objectKey = (pivot.object) ? _createResourceKey(activitySeed.objectResource) : '';
            var targetKey = (pivot.target) ? _createResourceKey(activitySeed.targetResource) : '';
            keys[route].push(util.format('%s#%s#%s#%s#%s', activityType, route, actorKey, objectKey, targetKey));
        }
    }
...
var _createResourceKey = function(resource) {
    return (resource) ? util.format('%s:%s', resource.resourceType, resource.resourceId) : RESOURCE_KEY_EMPTY;
};
...
var _produceAllRoutes = function(activitySeed, callback) {
    log().trace({'activitySeed': activitySeed}, 'Producing routes for activity resources.');
...
var _aggregationInterval = function() {
    var client = Redis.getClient();
    var now = Date.now();

    // Determine if the next processing should occur
    client.get('oae-activity:aggregateNextProcess', function(err, nextProcess) {
        if (err) {
            log().warn({'err': err}, 'Error determine next processing time for aggregation processing. Skipping interval.');
            return _setAggregationInterval();
        }

        if (nextProcess && nextProcess <= now) {
            // We should try and process. Attempt to get a lock
            client.setnx('oae-activity:aggregateProcessingLock', function(err, reply) {
                if (err) {
                    log().warn({'err': err}, 'Error acquiring aggregate lock for aggregation processing. Skipping interval.');
                    return _setAggregationInterval();
                }

                if (reply === 1) {
                    // Do the aggregate processing step
                    _processAllAggregates(function(err) {
                        if (err) {
                            log().error({'err': err}, 'Error processing aggregation')
                        }

                        return _setAggregationInterval();
                    });
                } else {
                    // Someone beat us to it (it was probably a race on the interval). Leave it be.
                    return _setAggregationInterval();
                }
            });
        }
...
var _setAggregationInterval = function() {
    aggregationTimerId = setTimeout(_aggregationInterval, 1000);
};

var _processAllAggregates = function(callback) {
    ActivityDAO.iterateAllAggregates(1000, function(aggregates, done) {

        // Build a list of aggregates that need collection, and also aggregates that need to be expired
        var aggregatesToExpire = [];
        var aggregatesToCollect = [];
        aggregates.forEach(function(aggregate) {
            // Mark it as an aggregate to collect if it is active (e.g., has more than 1 aggregate entry) and has been updated since the last collection
            if (aggregate.active === '1' && (!aggregate.lastCollection || aggregate.lastCollection < aggregate.lastModified)) {
                aggregatesToCollect.push(aggregate);
            }

            // Flag it to be expired if it is beyond its expiry.
            if (Date.now() - aggregate.lastModified > AGGREGATE_EXPIRY_IN_MS) {
                // There is a chance that it may be updated between this point, and when we actually delete it. Such aggregated activities
                // are effectively "lost" and never delivered to the stream. NoSQL be a harsh mistress :(
                aggregatesToExpire.push(aggregate);
            }
        });

        _collectAggregates(aggregatesToCollect, function(err) {
            if (err) {
                log().warn({'err': err}, 'Error collecting aggregates.');
            }
        });
...
var _collectAggregates = function(aggregates, callback) {
    if (aggregates.length === 0) {
        return callback();
    }

    log().trace({'aggregates': aggregates}, 'Collecting aggregates.');
    ...
    ActivityDAO.getAggregatedEntities(aggregateKeys, function(err, aggregatedEntities) {
        if (err) {
            return callback(err);
        }
        ...
        _.keys(aggregatedEntities).forEach(function(aggregateEntityKey) {
            
            // The aggregateEntityKey is basically a aggregateKey, but discriminated by the entity type, as it holds a list
            // of aggregated entities: <aggregateKey>#<entityType>; where entityType is one of "actor", "object" or "target"

            // Break the aggregateEntityKey into the aggregate key part and entity type part (<aggregate key>#<entity type>)
            var aggregateKey = aggregateEntityKey.split('#');
            var activityType = aggregateKey[0];
            var entityType = aggregateKey[aggregateKey.length - 1];
            aggregateKey = aggregateKey.slice(0, aggregateKey.length - 1).join('#');

            if (!aggregateActivities[aggregateKey]) {
                // Generate a new ID for this aggregate activity
                var activityId = ShortId.generate();
                aggregateActivities[aggregateKey] = new Activity(activityType, activityId, 
            }
        });
    });
};