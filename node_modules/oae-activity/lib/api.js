...
var registerActivityType = module.exports.registerActivityRouter = function(activityType, groupBy) {
...
var _handleActivityTask = function(activitySeed, callback) {
    ...
    _produceAllRoutes(activitySeed, actor, object, target, function(err, actorRoutes, objectRoutes, targetRoutes) {
        ...
        _makeAggregate(activitySeed, function(err, activeRoutes) {
            if (err) {
                return callback(err);
            } else if (isAggregating) {
                // If the feed is already aggregating do not push the activities to the feeds, this will be done on a timed interval
                return callback();
            }
...
var _makeAggregates = function(routes, activitySeed, callback) {
    var keys = _createAggregateKeys(routes, activitySeed);

    // Get the last modified date of all aggregates, this will determine which are already aggregating
    PrincipalDAO.getAggregateLastModifiedByKeys(keys, function(err, lastModified) {
        if (err) {
            return callback(err);
        }

        PrincipalDAO.updateAggregateLastModifiedByKeys
...
var _createAggregateKeys = function(routes, activitySeed) {
    var keys = [];
    var groupBy = activityTypes[activitySeed.activityType] || [];
    ...
    for (var groupByIndex = 0; groupByIndex < groupBy.length; groupByIndex++) {
        ...
        for (var routeIndex = 0; routeIndex < routes.length; routeIndex++) {
            ...
            keys.push(util.format('%s#%s#%s#%s#%s', activityType, route, actorKey, objectKey, targetKey));
        }
    }
