...
var _handleIndexChange = function(ctx, followingUserId, followedUserId) {
    var indexTasks = [
        new IndexTask('following', {'id': followingUserId}),
        new IndexTask('followers', {'id': followedUserId})
    ];
    SearchAPI.submitIndexTasks(indexTasks);
};

/**
 * Produces a search child documents that contain the followers of a user
 *
 * @see SearchAPI#registerSearchDocumentProducer
 * @api private
 */
var _produceFollowersDocuments = function(data, callback) {
    if (!data || !data.id) {
        return callback();
    }

    FollowingDAO.getFollowers(data.id, null, 10000, function(err, followerUserIds) {
        if (err) {
            return callback(err);
        }
        ...
        var doc = {
            '_parent': data.id,
            '_type': 'followers',
            'id': data.id + '-followers',
            'followers_ids': followerUserIds
        };
...
/**
 * Produces a search child documents that contain the users a user follows
 *
 * @see SearchAPI#registerSearchDocumentProducer
 * @api private
 */
var _produceFollowingDocuments = function(data, callback) {
    if (!data || !data.id) {
        return callback();
    }

    FollowingDAO.getFollowing(data.id, null, 10000, function(err, followingUserIds) {
        if (err) {
            return callback(err);
        }
        ...
        var doc = {
            '_parent': data.id,
            '_type': 'following',
            'id': data.id + '-following',
            'following_ids': followingUserIds
        };
...
SearchAPI.registerSearchDocumentProducer('following', _produceFollowingDocuments);
SearchAPI.registerSearchDocumentProducer('followers', _produceFollowersDocuments);
