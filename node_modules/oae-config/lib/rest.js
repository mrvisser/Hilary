...
var _handleGetConfig = function(req, res) {
    // Get the etag version of the request
    var version = OaeUtil.parseIfNoneMatch(req);

    // If the tenant alias is specified in the request parameters, use it.
    // Otherwise we use the current tenant in context
    var tenantAlias = req.params.tenantAlias || req.ctx.tenant().alias;
    ConfigAPI.getTenantConfig(req.ctx, tenantAlias, {'unlessVersion': version}, function(err, config, version) {
        if (err) {
            return res.send(err.code, err.msg);
        }

        // Set the configuration etag version on the response
        OaeUtil.setEtag(res, version);
