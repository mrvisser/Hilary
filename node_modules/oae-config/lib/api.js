...
// Will be used to cache the global and tenant schemas
var cachedGlobalSchema = {};
var cachedTenantSchema = {};

// Will be used as a version salt for the current version of a tenant
// configuration. This is needed because we need to change the version of a
// tenant's configuration if new elements are added or default values are
// changed
var cachedSchemaVersion = '';

// Will be used to hold the cached admin tenant configuration values
var cachedGlobalConfig = {};

// Will be used as a size-bounded LRU cache for directly fetched tenant
// configuration values
var tenantConfigValueCache = null;
 ...
 *
 * @param  {String}  messageJson    The message sent out to the cluster, provides the tenant alias and configKeys that were changed
 */
Pubsub.on('oae-config', function(messageJson) {
    var message = JSON.parse(messageJson);
    var tenantAlias = message.alias;
    var configKeys = message.configKeys;

    var isAdminTenant = (tenantAlias === config.servers.globalAdminAlias);
    OaeUtil.invokeIfNecessary(isAdminTenant, _cacheGlobalConfig, function(err) {
        if (err) {
            return log().error({'err': err, 'tenantAlias': tenantAlias}, 'An error occurred recaching the global admin tenant configuration');
        } else if (!isAdminTenant) {
            tenantConfigValueCache.invalidate(tenantAlias, configKeys);
        }

        return ConfigAPI.emit('update', tenantAlias, configKeys);
    });
});
...
module.exports.config = function(moduleId) {
    ...
    return {
        ...
        /**
         * Get a configuration value for the current module. This internal
         * function will also return suppressed values, as they will be
         * necessary for usage inside of the internal APIs
         *
         ...
         * @param  {Any}        callback.value      The requested config value e.g. `true`. This will be null if the config element cannot be found
         */
        'getValue': function(tenantAlias, featureKey, elementKey, callback) {
            var configValueInfo = _resolveConfigValueInfo(tenantAlias, moduleId, featureKey, elementKey);
            if (configValueInfo) {
                return callback(null, configValueInfo.value);
            } else {
                return callback();
            }
        },
        ...
        /**
         * Get the configuration values for the current module. This internal
         * function will also return suppressed values, as they will be
         * necessary for usage inside of the internal APIs
         *
         * @param  {String}     tenantAlias         The alias of the tenant for which to get the config value
         * @param  {String}     featureKey          The feature to get the element's value for. e.g., `twitter`
         * @param  {String[]}   elementKeys         The elements to get the config value for. e.g., `enabled`
         * @param  {Function}   callback            Standard callback function
         * @param  {Object}     callback.err        An error that occurred, if any
         * @param  {Object}      callback.values    The requested config values keyed by element key
         */
        'getValues': function(tenantAlias, featureKey, elementKeys, callback) {
            var result = _.chain(elementKeys)
                .map(function(elementKey) {
                    var configValueInfo = _resolveConfigValueInfo(tenantAlias, moduleId, featureKey, elementKey);
                    if (configValueInfo) {
                        return [elementKey, configValueInfo.value];
                    }
                })
                .compact()
                .object()
        ...
        /**
         * Get the value for the specified element for all tenants
         */
        'getAllValues': function(featureKey, elementKey, callback) {
            var configKey = _generateColumnKey(moduleId, featureKey, elementKey);
            var element = _element(moduleId, featureKey, elementKey);
            if (!element) {
                return callback({'code': 404, 'msg': 'There is no config value at location: %s', configKey});
            }

            Cassandra.runQuery('SELECT "value" FROM "Config" WHERE "configKey" = ? ALLOW FILTERING', [configKey], function(err, rows) {
                if (err) {
                    return callback(err);
                }

                var config = _rowsToConfig(rows);

                // Find the default tenant value
                var defaultValue = config[config.servers.globalAdminAlias];
                if (_.isUndefined(defaultValue)) {
                    defaultValue = element.defaultValue;
                }
                delete config[config.servers.globalAdminAlias];
         ...
         */
        'getLastUpdated': function(tenantAlias, feature, element, callback) {
            ...
            return callback(null, lastUpdated || new Date(0));
        }
    };
};
 ...
 * @param  {String}     [opts.unlessVersion]        If the current version of the configuration is this provided version, then a 304 response is given in as an `err` instead of a configuration. If unspecified, the configuration is always returned
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occurred, if any
 * @param  {Object}     callback.config             JSON object representing the full cached tenant config
 * @param  {String}     callback.version            The current version of the tenant configuration
 */
var getTenantConfig = module.exports.getTenantConfig = function(ctx, tenantAlias, opts, callback) {
    opts = opts || {};
    ...
    // TODO check cached version here before reaching to Cassandra

    var isGlobalTenant = (tenantAlias === config.servers.globalTenantAlias);
    OaeUtil.invokeIfNecessary(!isGlobalTenant, _getPersistentTenantConfig, function(err, tenantConfigValues) {
        if (err) {
            return callback(err);
        }
        ...
        _.each(cachedGlobalSchema, function(module, moduleKey) {
            _.each(module, function(feature, featureKey) {
                _.each(feature.elements, function(element, elementKey) {
                    ...
                    var configValueInfo = _resolveConfigValueInfo(tenantConfigValues, moduleKey, featureKey, elementKey);
                    ...
                    var timestampMillis = configValueInfo.timestamp.getTime();
        ...
        var version = _.oaeHashCode(util.format('%s:%s', cachedSchemaVersion, latestTimestamp));

        // If the current version is the same as the one specified, if any, then we
        // return a 304
        if (opts.unlessVersion === version) {
            return callback({'code': 304, 'msg': 'Not modified'});
        }

        // Otherwise, we return the information
        return callback(null, tenantConfig, version);
    });
};
 ...
 *
 * @param  {Object}     tenantConfigValues          The overriding configuration values for the tenant
 ...
 * @api private
 */
var _resolveConfigValueInfo = function(tenantConfigValues, moduleKey, featureKey, elementKey) {
    tenantConfigValues = tenantConfigValues || {};
...
var initConfig = module.exports.initConfig = function(_config, callback) {
    ...
    _cacheSchema(function() {
        ...
        // Calculate the version string for client-side configuration cache
        cachedSchemaVersion = _.oaeHashCode({
            'global': cachedGlobalSchema,
            'tenant': cachedTenantSchema
        });

        console.log('version: %s', cachedSchemaVersion);

        return _cacheAllTenantConfigs(callback);
    });
};
...
var getPersistentConfigValuesForTenants = module.exports.getPersistentConfigValuesForTenants = function(aliases, configKeys, callback) {
    _getPersistentConfigValuesForTenants(aliases, configKeys, function(err, persistentConfigsByTenant) {
        if (err) {
            return callback(err):
        }
    });
 ...
 * @api private
 */
var _cacheGlobalConfig = function(callback) {
    // Indicate that config values are about to be re-cached
    ConfigAPI.emit('preCache');
    _getPersistentTenantConfig(config.servers.globalTenantAlias, function(err, globalTenantConfig) {
        if (err) {
            return callback(err);
        }

        cachedGlobalConfig = globalTenantConfig;
...
var _getPersistentConfigValuesForTenants = function(aliases, configKeys, callback) {
    var cql = 'SELECT "tenantAlias", "configKey", "value", WRITETIME("value") FROM "Config" WHERE "tenantAlias" IN (?) AND "configKey" IN (?)';
    var params = [aliases, configKeys];
...
var updateConfig = module.exports.updateConfig = function(ctx, tenantAlias, configValues, callback) {
    ...
    var isGlobalTenant = (tenantAlias === config.servers.globalTenantAlias);
    OaeUtil.invokeIfNecessary(isGlobalTenant, _getPersistentTenantConfig, tenantAlias, function(err, tenantConfigValues) {
        if (err) {
            return callback(err);
        }
        ...
        _.each(configValues, function(value, key) {
            ...
            if (optionalKey) {
                var currentConfigValue = _resolveConfigValueInfo(tenantConfigValues, module, feature, element).value;
        ...
        Cassandra.runBatchQuery(queries, function(err) {
            ...
            Pubsub.publish('oae-config', JSON.stringify({'alias': tenantAlias, 'configKeys': _.keys(aggregatedValues)});
            return callback();
        });
    });
};
