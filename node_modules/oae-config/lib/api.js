...
module.exports.config = function(moduleId) {
    ...
    return {
        ...
        'getValue': function(tenantAlias, featureKey, elementKey, callback) {
            ...
            if (configValueInfo) {
                return setImmediate(callback, null, configValueInfo.value);
            } else {
         ...
         * @param  {Function}   callback            Standard callback function
         * @param  {Object}     callback.err        An error that occurred, if any
         * @param  {Object}     callback.values     An object keyed by element key, where the value is the requested config value e.g. `true`. If an element could not be found, `null` will be in place of a value
         */
        'getValues': function(tenantAlias, featureKey, elementKeys, callback) {
            var values = _.chain(elementKeys)
                .map(function(elementKey) {
                    var configValueInfo = _resolveConfigValueInfo(tenantAlias, moduleId, featureKey, elementKey);
                    var value = null;
                    if (configValueInfo) {
                        value = configValueInfo.value;
                    }

                    return [elementKey, value];
                })
                .object()
                .value();
         ...
         * @param  {Function}   callback                Standard callback function
         * @param  {Object}     callback.err            An error that occurred, if any
         * @param  {Date}       callback.lastUpdated    The timestamp when the config value was last updated. If the element could not be found, the epoch date (0 milliseconds) will be returned
         */
