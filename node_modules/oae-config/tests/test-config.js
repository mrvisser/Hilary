...
describe('Configuration', function() {
    ...
    describe('Internal Configuration API', function() {
        ...
        it('verify get single config value', function(callback) {
            ...
            AuthenticationConfig.getValue(global.oaeTests.tenants.cam.alias, 'sso', 'enabled', function(err, value) {
                assert.ok(!err);
                assert.strictEqual(value, null);

                // Retrieve a boolean value
                AuthenticationConfig.getValue(global.oaeTests.tenants.cam.alias, 'twitter', 'enabled', function(err, value) {
                    assert.ok(!err);
                    assert.strictEqual(value, true);

                    // Retrieve a single string value
                    PrincipalsConfig.getValue(global.oaeTests.tenants.cam.alias, 'user', 'defaultLanguage', function(err, value) {
                        assert.ok(!err);
                        assert.strictEqual(value, 'en_GB');

                        // Retrieve a suppressed value
                        PrincipalsConfig.getValue(global.oaeTests.tenants.cam.alias, 'recaptcha', 'privateKey', function(err, value) {
                            assert.ok(!err);
                            assert.strictEqual(value, '6LcFWdYSAAAAANrHjt2Y5VJXoICHa95PFDarVcGs');
                            return callback();
                        });
                    });
                });
        ...
        it('verify validation', function(callback) {
            ...
            // TODO: Return an actual `err` now that we use a callback

            // Verify that a feature needs to be provided when getting a config value
            var AuthenticationConfig = ConfigAPI.config('oae-authentication');
            AuthenticationConfig.getValue(global.oaeTests.tenants.cam.alias, undefined, undefined, function(err, value) {
                assert.ok(!err);
                assert.strictEqual(value, null);

                // Verify that an element needs to be provided when getting a config value
                AuthenticationConfig.getValue(global.oaeTests.tenants.cam.alias, 'twitter', undefined, function(err, value) {
                    assert.ok(!err);
                    assert.strictEqual(value, null);
                    return callback();
                });
        ...
        /**
         * Test that verifies validation of the last updated timestamp
         */
        it('verify validation of getting the last updated timestamp', function(callback) {
            var PrincipalsConfig = ConfigAPI.config('oae-principals');

            // TODO: Return actual `err` for this instead now that we use callbacks

            // Not passing in any of the `tenantAlias`, `feature` or `element`
            // parameters should result in the epoch date being returned
            PrincipalsConfig.getLastUpdated(undefined, undefined, undefined, function(err, lastUpdated) {
                assert.ok(!err);
                assert.strictEqual(lastUpdated.getTime(), 0);
                PrincipalsConfig.getLastUpdated(global.oaeTests.tenants.cam.alias, undefined, undefined, function(err, lastUpdated) {
                    assert.ok(!err);
                    assert.strictEqual(lastUpdated.getTime(), 0);
                    PrincipalsConfig.getLastUpdated(global.oaeTests.tenants.cam.alias, 'recaptcha', undefined, function(err, lastUpdated) {
                        assert.ok(!err);
                        assert.strictEqual(lastUpdated.getTime(), 0);

                        // Passing in an unknown element should result in the epoch date
                        PrincipalsConfig.getLastUpdated(global.oaeTests.tenants.cam.alias, 'careful', 'now', function(err, lastUpdated) {
                            assert.ok(!err);
                            assert.strictEqual(lastUpdated.getTime(), 0);
        ...
        /**
         * Test that verifies the last updated tiemstamp increases when Updated
         */
        it('verify the last updated timestamp increases when updated', function(callback) {
            var PrincipalsConfig = ConfigAPI.config('oae-principals');

            // The createTenantWithAdmin test utility will set the recaptcha
            // value to disabled when the tenant has been created. This should
            // be reflected in the last updated timestamp
            ...
            TestsUtil.createTenantWithAdmin(tenantAlias, tenantAlias, function(err, tenant, tenantAdminRestContext) {
                ...
                PrincipalsConfig.getLastUpdated(tenantAlias, 'recaptcha', 'enabled', function(err, recaptchaUpdate) {
                    assert.ok(!err);
                    assert.ok(recaptchaUpdate);

                    var recaptchaUpdateTime = recaptchaUpdate.getTime();
                    assert.ok(recaptchaUpdateTime > 0);

                    // Enable recaptcha on the tenant
                    setTimeout(ConfigTestUtil.updateConfigAndWait, tenantAdminRestContext, null, {'oae-principals/recaptcha/enabled': true}, function(err) {
                        assert.ok(!err);

                        // Ensure the config value update time is larger than it was before
                        PrincipalsConfig.getLastUpdated(tenantAlias, 'recaptcha', 'enabled', function(err, recaptchaUpdate) {
                            assert.ok(!err);
                            assert.ok(recaptchaUpdate.getTime() > recaptchaUpdateTime);
            ...
            TestsUtil.createTenantWithAdmin(tenantAlias, tenantAlias, function(err, tenant, tenantAdminRestContext) {
                ...
                PrincipalsConfig.getLastUpdated(tenantAlias, 'recaptcha', 'enabled', function(err, recaptchaUpdate) {
                    ...
                    ConfigTestUtil.updateConfigAndWait(tenantAdminRestContext, null, {'oae-authentication/twitter/enabled': true}, function(err) {
                        ...
                        PrincipalsConfig.getLastUpdated(tenantAlias, 'recaptcha', 'enabled', function(err, recaptchaUpdate) {
                            assert.ok(!err);
                            assert.strictEqual(recaptchaUpdate.getTime(), recaptchaUpdateTime);
                            return callback();
                        });
                    });
                });
