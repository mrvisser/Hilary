...
var _retrieveThumbnails = function(ctx, contentItems, callback, _paths) {
    ...
    ContentUtil.getStorageBackend(ctx, contentItem.previews.thumbnailUri, function(err, backend) {
        if (err) {
            return callback(err);
        }

        backend.get(ctx, contentItem.previews.thumbnailUri, function(err, file) {
            if (err) {
                return callback(err);
            }
...
var _removeOldPreview = function(ctx, folder, type, callback) {
    ...
    ContentUtil.getStorageBackend(ctx, null, function(err, backend) {
        ...
        return backend.remove(ctx, folder.previews[type], callback);
    });
};
...
var _storeNewPreviews = function(ctx, folder, thumbnail, wide, callback) {
    ContentUtil.getStorageBackend(ctx, null, function(err, backend) {
        ...
        backend.store(ctx, thumbnail, {'filename': filename, 'resourceId': folder.id}, function(err, thumbnailUri) {
            if (err) {
                return callback(err);
            }

            filename = util.format('wide_%s.jpg', Date.now());
            backend.store(ctx, wide, {'filename': filename,'resourceId': folder.id}, function(err, wideUri) {
                if (err) {
                    return callback(err);
                }
