...
var test = module.exports.test = function(ctx, contentObj, callback) {
    ...
    // If it's not a youtube URL, we don't process it
    var isYoutubeUrl = (YOUTUBE_FULL_REGEX.test(contentObj.link) || YOUTUBE_SHORT_REGEX.test(contentObj.link));
    if (!isYoutubeUrl) {
        return callback(null, -1);
    }

    // Check if we're configured to deal with Youtube URLs
    PreviewConfig.getValue('admin', 'youtube', 'key', function(err, key) {
        if (err) {
            return callback(err);
        } else if (!key) {
            return callback(null, -1);
...
var generatePreviews = module.exports.generatePreviews = function(ctx, contentObj, callback) {
    ...
    PreviewConfig.getValue('admin', 'youtube', 'key', function(err, youtubeKey) {
        if (err) {
            return callback(err);
        }
