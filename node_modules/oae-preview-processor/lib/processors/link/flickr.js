...
var test = module.exports.test = function(ctx, contentObj, callback) {
    ...
    // Only handle flickr urls
    var isFlickrUrl = (REGEX_PHOTO.test(contentObj.link) || REGEX_SHORT_PHOTO.test(contentObj.link) || REGEX_SET.test(contentObj.link));
    if (!isFlickrUrl) {
        return callback(null, -1);
    }

    // Check that this retriever has been configured in the Admin UI
    _getConfig(function(err, config) {
        if (err) {
            return callback(err);
 ...
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred, if any
 * @param  {Object}     callback.flickrConfig   The apikey and sharedsecret from the Admin UI
 * @api private
 */
var _getConfig = function(callback) {
    return PreviewConfig.getValues('admin', 'flickr', ['apikey', 'apisecret'], callback);
};
