...
var test = module.exports.test = function(ctx, contentObj, callback) {
    ...
    // Don't handle it if it isn't a slideshare url
    var isSlideshareUrl = SLIDES_REGEX.test(contentObj.link);
    if (!isSlideshareUrl) {
        return callback(null, -1);
    }
    ...
    _getConfig(function(err, config) {
        if (err) {
            return callback(err);
        } else if (!config.apikey || !config.sharedsecret) {
            return callback(null, -1);
        }
 ...
 * @param  {Object}     callback.slideshareConfig   The apikey and sharedsecret from the Admin UI
 * @api private
 */
var _getConfig = function(callback) {
    PreviewConfig.getValues('admin', 'slideshare', ['apikey', 'sharedsecret'], callback);
};
