 ...
 */
var refreshStrategies = module.exports.refreshStrategies = function(tenant, callback, _strategies) {
    callback = callback || function(err) {
        if (err) {
            log().error({
                'err': err,
                'tenant': tenant
            }, 'An error occurred refreshing authentication strategies for tenant');
        }
    };
    _strategies = _strategies || _.map(strategies, function(strategy, strategyName) {
        return {
            'name': strategyName,
            'strategy': strategy
        };
    });
    if (_.isEmpty(_strategies)) {
    ...
    var strategyInfo = _strategies.shift();
    var strategyName = strategyInfo.name;
    ...
    strategy.shouldBeEnabled(tenant.alias, function(err, shouldBeEnabled) {
        if (err) {
            log().error({
                'err': err,
                'strategyName': strategyName
            }, 'An error occurred while determining if a strategy should be enabled');
            return refreshStrategies(tenant, callback, _strategies);
        }

        OaeUtil.invokeIfNecessary(shouldBeEnabled, strategy.getPassportStrategy, tenant, function(err, tenantStrategy) {
            if (err) {
                return callback(err)
            } else if (shouldBeEnabled) {
                // If the tenant wants the strategy enabled, we enable it. We also
                // create a new instance of the passport strategy so that
                // configuration updates to the strategy are taken into account
                passport.use(passportStrategyName, tenantStrategy);
...
var _refreshAllTenantStrategies = function() {
    ...
    _.each(_.values(tenants), function(tenant) {
        refreshStrategies(tenant);
    });
};
