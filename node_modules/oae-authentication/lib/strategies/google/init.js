...
module.exports = function() {
    ...
    strategy.shouldBeEnabled = function(tenantAlias, callback) {
        return AuthenticationConfig.getValue(tenantAlias, AuthenticationConstants.providers.GOOGLE, 'enabled', callback);
    };
    ...
    strategy.getPassportStrategy = function(tenant, callback) {
        ...
        AuthenticationConfig.getValues(tenant.alias, AuthenticationConstants.providers.GOOGLE, ['key', 'secret', 'domains'], function(err, authConfig) {
            if (err) {
                return callback(err);
            }

            // Ensure we can do simple string comparisons by filtering empty domains and trimming out spaces
            authConfig.domains = _.chain(authConfig.domains.split(','))
                .map(function(domain) {
            ...
            var passportStrategy = new GoogleStrategy({
                ...
                if (!_.isEmpty(authConfig.domains)) {
                    var emailDomain = email.split('@')[1];
                    if (!_.contains(authConfig.domains, emailDomain)) {
                        var err = {
                            'code': 400,
