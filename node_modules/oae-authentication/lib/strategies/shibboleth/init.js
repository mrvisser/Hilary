...
module.exports = function(config) {
    ...
    strategy.getPassportStrategy = function(tenant, callback) {
        ...
        AuthenticationConfig.getValue(tenant.alias, AuthenticationConstants.providers.SHIBBOLETH, 'idpEntityID', function(err, idpEntityID) {
            if (err) {
                return callback(err);
            }
            ...
            var passportStrategy = new ShibbolethStrategy({
                ...
                var mapConfigKeys = ['externalIdAttributes', 'mapDisplayName',
                    'mapEmail', 'mapLocale'];

                AuthenticationConfig.getValue(tenantAlias, AuthenticationConstants.providers.SHIBBOLETH, mapConfigKeys, function(err, authConfig) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    var externalId = _getBestAttributeValue(authConfig.externalIdAttributes, headers, headers['remote_user']);
                    if (!externalId) {
                    ...
                    var displayName = _getBestAttributeValue(authConfig.mapDisplayName, headers, headers['remote_user']);
                    ...
                    opts.email = _getBestAttributeValue(authConfig.mapEmail, headers);

                    // Get a locale, if any
                    var locale = _getBestAttributeValue(authConfig.mapLocale, headers, headers['locale']);
                    if (locale) {
...
/**
 * Get the values from the attribute that best matches a configured priority
 * list
 *
 * @param  {String}     configValue         The raw, space-separated priority list string
 ...
 * @api private
 */
var _getBestAttributeValues = function(configValue, headers, defaultValue) {
    priorityList = _.chain(configValue.split(' '))
        .compact()
        .uniq()
        .value();
