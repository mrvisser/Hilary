 ...
 * @param  {Boolean}    callback.isEnabled  `true` if the strategy is enabled, `false` otherwise
 */
var isEnabled = module.exports.isEnabled = function(tenantAlias, callback) {
    return AuthenticationConfig.getValue(tenantAlias, AuthenticationConstants.providers.SHIBBOLETH, 'enabled', callback);
};
...
var getShibbolethEnabledTenant = module.exports.getShibbolethEnabledTenant = function(tenantAlias, callback) {
    ...
    isEnabled(tenant.alias, function(err, isEnabled) {
        if (err) {
            return callback(err);
        } else if (!isEnabled) {
