...
var DEFAULT_TASK_RETRY_ATTEMPTS = 10;
var DEFAULT_TASK_RETRY_DELAY_MS = 1000;

var taskMaxRetryAttempts = DEFAULT_TASK_RETRY_ATTEMPTS;
var taskRetryDelayMs = DEFAULT_TASK_RETRY_DELAY_MS;
...
var init = module.exports.init = function(mqConfig, callback) {
    ...
    taskMaxRetryAttempts = OaeUtil.getNumberParam(mqConfig.tasks.taskRetryAttempts, 10);
    taskRetryDelayMs = OaeUtil.getNumberParam(mqConfig.tasks.taskRetryDelayMs, 1000);
 ...
 * @param  {Boolean}    [callback.failed]           A boolean, specifying whether or not there was a failure submitting the task.
 * @param  {Number}     [callback.retryAttempts]    How many times the task was attempted to be submit before this method called back
 */
var submit = module.exports.submit = function(name, data, callback) {
    MQ.emit('preSubmit', name, data);
    queueExchange.publish(name, data, Constants.DEFAULT_TASK_QUEUE_PUBLISH_OPTS, callback);
};

/**
 * Submit a job to the task queue, as per the #submit function.
 *
 * @param  {String}     name            Same as #submit
 * @param  {Object}     data            Same as #submit
 * @param  {Number}     retryAttempts   Number of retries that have been attempted
 * @param  {Function}   [callback]      Same as #submit
 * @api private
 */
var _submit = function(name, data, retryAttempts, callback) {
    callback = callback || function(failed, retryAttempts) {
        if (failed) {
            log().error({'name': name, 'data': data}, 'Failed to submit job after %s retries.', retryAttempts);
        }
    };

    if (retryAttempts > 0) {
        log().info({'data': data}, 'Trying to resubmit a task on queue "%s" after %s failure(s)', name, retryAttempts);
    }

    // We publish with the routing key `name`, which routes it to the queue (if any) that is listening on this key
    queueExchange.publish(name, data, Constants.DEFAULT_TASK_QUEUE_PUBLISH_OPTS, function(failed) {
        if (failed && retryAttempts >= taskMaxRetryAttempts) {
            return callback(true, retryAttempts);
        } else if (failed) {
            // If we have retry attempts left, try it again
            log().error({'data': data}, 'Error publishing task');
            return setTimeout(_submit, taskRetryDelayMs, name, data, retryAttempts + 1, callback);
        }

        return callback(false, retryAttempts);
    });
};
