...
/**
 * Parse the value of a potential "if-none-match" header from the given request
 *
 * @param  {Request}    req     The express request whose if-none-match header to parse
 * @return {String}             The value of the if-none-match tag, if any
 */
var parseIfNoneMatch = module.exports.parseIfNoneMatch = function(req) {
    var raw = req.headers['if-none-match'];
    if (raw) {
        if (raw[0] === 'W') {
            // If this is a weak etag, then it will be of the form
            // `W/"version_number"` where "version_number" is the desired result
            return raw.slice(3, -1);
        } else if (raw[0] === '"') {
            // It is a strong etag, which means it is in the form
            // `"version_number"`. Slice away the quotes
            return raw.slice(1, -1);
        }
    }

    // We don't have a valid if-none-match header
    return null;
};

/**
 * Set the given version value as the response ETag header, if specified
 *
 * @param  {Response}   response    The response on which to set the header
 * @param  {String}     [value]     The value to set as the etag version
 */
var setEtag = module.exports.setEtag = function(res, version) {
    if (_.isNull(version) || _.isUndefined(version)) {
        return;
    }

    res.setHeader('ETag', util.format('"%s"', version));

    // Force IE9 to revalidate it's cached version of this request. We can't
    // use `Cache-Control: no-cache` because this will force IE9 to not store
    // the response locally at all -- because it's broken. When IE9 is not
    // supported, this is no longer needed
