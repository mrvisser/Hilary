...
var runQuery = module.exports.runQuery = function(query, parameters, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({'err': err}, 'Error executing cassandra query');
        }
    };
    
    var isRead = (query.indexOf('SELECT') === 0);
    if (isRead) {
    ...
    var start = Date.now();
    executeQuery(query, parameters, function(err, rows) {
        if (err) {
            return callback(err);
        }

        if (isRead) {
            Telemetry.appendDuration('read.time', start);
        } else {
            Telemetry.appendDuration('write.time', start);
        }
