...
var executeQuery = function(query, parameters, callback) {
    ...
    // Set a timeout on the query. If it takes more than 30s we're going to bail, it probably error'd
    var queryTimeout = setTimeout(function() {
        returned = true;
        log().error({
            'query': query,
            'parameters': logParams
        }, 'Cassandra query took more than 5 seconds. Erroring.');
        return callback({'code': 500, 'msg': 'Query timed out, 5s'});
