...
var iterateAll = module.exports.iterateAll = function(columnNames, columnFamily, keyColumnName, opts, onEach, callback) {
    ...
    log().info({
        ...
        'returnKeyColumn': returnKeyColumn,
        'batchSize': opts.batchSize
    }, 'Beginning to iterate all rows of a column family');
 ...
 * @param  {String}     _fromKey            Used for recursion only. Specifies the key from which the next iteration batch should start
 * @param  {Number}     _count              Used for recursion and logging purposes only. Keeps a count of the the total number of rows iterated
 * @api private
 */
var _iterateAll = function(columnNames, columnFamily, keyColumnName, returnKeyColumn, batchSize, onEach, callback, _fromKey, _count) {
    _count = _count || 0;

    // Create and run the query to get the current set of results starting from the `_fromKey`
    var query = _buildIterateAllQuery(columnNames, columnFamily, keyColumnName, batchSize, _fromKey);
    runQuery(query.query, query.parameters, function(err, rows) {
        if (err) {
            ...
            log().info({
                ...
                'returnKeyColumn': returnKeyColumn,
                'batchSize': batchSize
            }, 'Finished iterating all %s rows of a column family', _count);

            // Notify the caller that we've finished
            return callback();
        }

        // Keep track of how many rows we fetched
        _count += rows.length;

        _.each(rows, function(row) {
            // Remember the last key to use for the next iteration
            _fromKey = row.get(keyColumnName).value;
        ...
        try {
            ...
            onEach(rows, function(err) {
                ...
                // Start the next iteration. Since `Cassandra.runQuery` jumps into a new process tick, there is no issue over this
                // recursion exceeding stack size with large data-sets
                return _iterateAll(columnNames, columnFamily, keyColumnName, returnKeyColumn, batchSize, onEach, callback, _fromKey, _count);
            });
        } catch (ex) {
