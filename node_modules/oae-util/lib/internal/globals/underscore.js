...
_.mixin({
    ...
    'oaeHashCode': function(obj) {
        var sha1 = crypto.createHash('sha1');
        sha1.update(_serialize(obj));
        return sha1.digest('hex');
    }
});

var _serialize = function(obj, _visited) {
    _visited = _visited || [];

    // Avoid circular recursion
    if (_.contains(_visited, obj)) {
        return '[circular]';
    }
    _visited.push(obj);
    ...
    var serialized = obj;
    if (_.isArray(obj)) {
        serialized = _.map(obj, function(val) {
            return _serialize(val, _visited);
        }).join(',');
    } else if (_.isObject(obj)) {
        // Convert the object into a consistent array and serialize it as an
        // array
        var arr = _.chain(obj)
            .keys()
            .sort()
            ...
            .value();
        serialized = _serialize(arr, _visited);
    }

    return util.format('[%s]', serialized);
};
