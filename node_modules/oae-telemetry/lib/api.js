...
var RateTelemetry = require('./internal/rate');
 ...
 *
 * @param  {Object}     telemetryConfig     The user-supplied telemetry configuration
 */
var init = module.exports.init = function(_telemetryConfig) {
    config = _telemetryConfig;
    if (config.enabled) {

        // Initialize rate information
        RateTelemetry.init(config.rate, Redis.getClient());

        // Initialize the correct publisher
        if (config.publisher) {
            var publisher = require('./publishers/' + config.publisher);
            publisher.init();
        }
    }
};
...
var Telemetry = function(module) {
    ...
    that.appendDuration = function(name, from) {
        ...
        stats.histograms[module][name].push(_duration(from));
    };
