...
var createUser = module.exports.createUser = function(ctx, tenantAlias, displayName, opts, callback) {
    ...
    PrincipalsConfig.getValues(tenantAlias, 'user', ['emailPreference', 'defaultLanguage', 'visibility'], function(err, principalsConfig) {
        if (err) {
            return callback(err);
        }

        var isAdmin = (ctx.user() && ctx.user().isAdmin(tenantAlias));
        opts.visibility = opts.visibility || principalsConfig.visibility;
        opts.publicAlias = opts.publicAlias || displayName;
        opts.acceptedTC = opts.acceptedTC || false;
        opts.emailPreference = opts.emailPreference || principalsConfig.emailPreference;

        if (!opts.locale) {
            if (!ctx.user()) {
                // If a user is creating an account for themself (i.e., they are
                // currently anonymous) then we try and use the locale suggested
                // by the request context (e.g., browser locale)
                opts.locale = ctx.locale();
            }

            // Finally, default to the tenant default if we couldn't get one
            // from the browser
            opts.locale = opts.locale || principalsConfig.defaultLanguage;
        }
        ...
        PrincipalsConfig.getValue(tenantAlias, 'termsAndConditions', 'enabled', function(err, needsToAcceptTC) {
            if (err) {
                return callback(err);
            }
            ...
            user.needsToAcceptTC = needsToAcceptTC;
...
var getMe = module.exports.getMe = function(ctx, callback) {
    ...
    TenantsUtil.isPrivate(tenant.alias, function(err, isPrivate) {
        if (err) {
            return callback(err);
        }

        tenant.isPrivate = isPrivate;
        ...
        getUser(ctx, ctx.user().id, function(err, data) {
            ...
            PrincipalsTermsAndConditionsAPI.needsToAcceptTermsAndConditions(ctx, function(err, needsToAcceptTC) {
                if (err) {
                    return callback(err);
                }

                data.needsToAcceptTC = needsToAcceptTC;
...
var _sendEmailToken = function(ctx, user, email, token, callback) {
    ...
    PrincipalsDAO.storeEmailToken(user.id, email, token, function(err) {
        ...
        require('oae-ui').getTenantSkinVariables(ctx.tenant().alias, function(err, tenantSkinVariables) {
            if (err) {
                return callback(err);
            }
