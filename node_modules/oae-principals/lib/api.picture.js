    ...
    ImageUtil.autoOrient(file.path, {'removeInput': true}, function(err, orientedFile) {
        ...
        ImageUtil.convertToJPG(orientedFile.path, function(err, convertedFile) {
            ...
            ContentUtil.getStorageBackend(ctx, null, function(err, backend) {
                ...
                backend.store(ctx.tenant().alias, convertedFile, options, function(err, largePictureUri) {
                    if (err) {
                        return _cleanupOnError(err, convertedFile, callback);
                    }
...
var _generateSizes = function(ctx, principal, x, y, width, callback) {
    ...
    ContentUtil.getStorageBackend(ctx, principal.picture.largeUri, function(err, backend) {
        if (err) {
            return callback(err);
        }

        backend.get(ctx.tenant().alias, principal.picture.largeUri, function(err, file) {
            if (err) {
                return callback(err);
            }
