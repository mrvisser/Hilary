...
var getPrincipals = module.exports.getPrincipals = function(principalIds, fields, callback) {
    ...
    Cassandra.runQuery(query, parameters, function(err, rows) {
        ...
        // Ensure callback is only invoked once between error and success
        // scenarios
        callback = _.once(callback);

        var principals = [];
        var _done = _.after(rows.length, function() {
            return callback(null, _.indexBy(principals, 'id'));
        });

        _.each(rows, function(row) {
            _getPrincipalFromRow(row, function(err, principal) {
                if (err) {
                    return callback(err);
                } else if (principal) {
...
var _getPrincipalFromCassandra = function(principalId, callback) {
    Cassandra.runQuery('SELECT * FROM "Principals" WHERE "principalId" = ?', [principalId], function (err, rows) {
        ...
        return _getPrincipalFromRow(rows[0], callback);
    });
};
...
var _getUserFromRedis = function(userId, callback) {
    Redis.getClient().hgetall(userId, function(err, hash) {
        ...
        return _hashToUser(hash, callback);
    });
};
 ...
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error that occurred, if any
 * @param  {User|Group}     callback.principal  A user or group object
 * @api private
 */
var _getPrincipalFromRow = function(row, callback) {
    if (row.count <= 1) {
        return callback();
    }

    var principalId = row.get('principalId').value;
    if (isGroup(principalId)) {
        return callback(null, _getGroupFromRow(row));
    }

    return _getUserFromRow(row, callback);
};
 ...
 * @api private
 */
var _getUserFromRow = function(row, callback) {
    var hash = Cassandra.rowToHash(row);
 ...
 * @api private
 */
var _hashToUser = function(hash, callback) {
    PrincipalsConfig.getValue(hash.tenantAlias, 'user', 'emailPreference', function(err, defaultEmailPreference) {
        if (err) {
            return callback(err);
        }

        return callback(null, new User(hash.tenantAlias, hash.principalId, hash.displayName, hash.email, {
            ...
            'emailPreference': (hash.emailPreference || defaultEmailPreference),
