...
/**
 * Transform persisted user activity entities that can be used in an activity
 * stream. The user property will be taken from the persisted entity, scrubbed
 * and returned as the "transformed entity"
 *
 * @param  {Context}    ctx                     Standard context object containing the current user and the current tenant
 * @param  {Object[]}   userInfos               The user info objects to transform
 * @param  {String}     userInfos[i].userId     The id of the user
 * @param  {User}       [userInfos[i].user]     The user object. If not specified, the generated entity with be abbreviated with just the information available
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred, if any
 * @param  {User}       callback.user           The scrubbed and transformed user object
 */
var transformPersistentUserActivityEntitiesToInternal = module.exports.transformPersistentUserActivityEntitiesToInternal = function(ctx, userInfos, callback, _userInfos, _entities) {
    _userInfos = _userInfos || userInfos.slice();
    _entities = _entities || [];
    if (_.isEmpty(_userInfos)) {
        return callback(null, _entities);
    ...
    var userInfo = _userInfos.shift();
    _transformPersistentUserActivityEntityToInternal(ctx, userInfo.userId, userInfo.user, function(err, entity) {
        if (err) {
            return callback(err);
        }

        _entities.push(entity);
        return transformPersistentUserActivityEntitiesToInternal(ctx, userInfos, callback, _userInfos, _entities);
    });
};
 ...
 * @api private
 */
var _transformPersistentUserActivityEntityToInternal = function(ctx, userId, user, callback) {
    if (!user) {
