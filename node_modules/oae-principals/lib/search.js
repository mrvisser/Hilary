/*!
 * Copyright 2013 Apereo Foundation (AF) Licensed under the
 * Educational Community License, Version 2.0 (the "License"); you may
 * not use this file except in compliance with the License. You may
 * obtain a copy of the License at
 *
 *     http://opensource.org/licenses/ECL-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an "AS IS"
 * BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */

var _ = require('underscore');

var AuthzAPI = require('oae-authz');
var AuthzConstants = require('oae-authz/lib/constants').AuthzConstants;
var AuthzUtil = require('oae-authz/lib/util');
var IndexTask = require('oae-search/lib/model').IndexTask;
var SearchAPI = require('oae-search');
var SearchConstants = require('oae-search/lib/constants').SearchConstants;
var SearchUtil = require('oae-search/lib/util');
var TenantsAPI = require('oae-tenants');

var PrincipalsAPI = require('oae-principals');
var PrincipalsConstants = require('oae-principals/lib/constants').PrincipalsConstants;
var PrincipalsDAO = require('oae-principals/lib/internal/dao');
var PrincipalsUtil = require('oae-principals/lib/util');

var User = require('oae-principals/lib/model').User;
var Group = require('oae-principals/lib/model').Group;



////////////////////
// INDEXING TASKS //
////////////////////

PrincipalsAPI.on(PrincipalsConstants.events.CREATED_USER, function(ctx, user) {
    var indexTasks = [
        new IndexTask('user', {'id': user.id})
    ];
    SearchAPI.submitIndexTasks(indexTasks);
});

PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_USER, function(ctx, user) {
    var indexTasks = [
        new IndexTask('user', {'id': user.id})
    ];
    SearchAPI.submitIndexTasks(indexTasks);
});

PrincipalsAPI.on(PrincipalsConstants.events.CREATED_GROUP, function(ctx, group, members) {
    var indexTasks = [
        new IndexTask('group', {'id': group.id}),
        new IndexTask('group_members', {'id': group.id})
    ];
    SearchAPI.submitIndexTasks(indexTasks);

    // Fire additional tasks to update the memberships of the members
    _fireMembershipUpdateTasks(_.keys(members));
});

PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_GROUP, function(ctx, group) {
    var indexTasks = [
        new IndexTask('group', {'id': group.id})
    ];
    SearchAPI.submitIndexTasks(indexTasks);
});

PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, function(ctx, group, memberUpdates) {
    _handleUpdateGroupMembers(ctx, group, _.keys(memberUpdates));
});

PrincipalsAPI.on(PrincipalsConstants.events.JOINED_GROUP, function(ctx, group) {
    _handleUpdateGroupMembers(ctx, group, [ctx.user().id]);
});

PrincipalsAPI.on(PrincipalsConstants.events.LEFT_GROUP, function(ctx, group) {
    _handleUpdateGroupMembers(ctx, group, [ctx.user().id]);
});

/**
 * Submits the indexing operation required when a group's members have changed.
 *
 * @param  {Context}    ctx             The execution context of the request that resulted in the change event
 * @param  {Group}      group           The group object whose membership changed
 * @param  {String[]}   principalIds    The ids of all the members whose status in the group changed
 * @api private
 */
var _handleUpdateGroupMembers = function(ctx, group, principalIds) {
    var indexTasks = [
        new IndexTask('group_members', {'id': group.id})
    ];
    SearchAPI.submitIndexTasks(indexTasks);

    // Fire additional tasks to update the memberships of the members
    _fireMembershipUpdateTasks(principalIds);
};

/**
 * Given a list of ids of principals whose membership has changed, fire the index tasks required to update the appropriate
 * membership documents.
 *
 * @param  {String[]}  principalIds    The list of principals whose membership in a group changed.
 * @api private
 */
var _fireMembershipUpdateTasks = function(principalIds) {
    if (principalIds && principalIds.length > 0) {

        var indexTasks = [];
        _.each(principalIds, function(principalId) {
            if (PrincipalsUtil.isGroup(principalId)) {
                indexTasks.push(new IndexTask('group_memberships', {'id': principalId}));
            } else {
                indexTasks.push(new IndexTask('user_memberships', {'id': principalId}));
            }
        });

        if (indexTasks.length > 0) {
            SearchAPI.submitIndexTasks(indexTasks);
        }
    }
};



/////////////////////
// CUSTOM SEARCHES //
/////////////////////

SearchAPI.registerSearch('members', require('./searches/members'));
SearchAPI.registerSearch('memberships', require('./searches/memberships'));



////////////////////////
// DOCUMENT PRODUCERS //
////////////////////////

/**
 * Produces a search documents for a 'user' or a 'group'
 *
 * @see SearchAPI#registerSearchDocumentProducer
 * @api private
 */
var _producePrincipalDocument = function(data, callback) {
    PrincipalsDAO.getPrincipal(data.id, function(err, principal) {
        if (err) {
            return callback(err);
        } else if (PrincipalsUtil.isGroup(principal.id)) {
            var doc = _produceGroupDocument(principal);
            return callback(null, doc);
        } else {
            var doc = _produceUserSearchDocument(principal);
            return callback(null, doc);
        }
    });
};

/**
 * Given a group, produces a SearchDocument
 *
 * @param  {Group}              group   The group for which to produce a search document
 * @return {SearchDocument}             A search document
 * @api private
 */
var _produceGroupDocument = function(group) {
    var fullText = _.compact([group.displayName, group.alias, group.description]).join(' ');
    var doc = {
        'resourceType': group.resourceType,
        'id': group.id,
        'tenantAlias': group.tenant.alias,
        'displayName': group.displayName,
        'visibility': group.visibility,
        'joinable': group.joinable,
        'q_high': group.displayName,
        'q_low': fullText,
        'sort': fullText,
        '_extra': {
            'alias': group.alias
        }
    };

    if (group.picture.mediumUri) {
        doc.thumbnailUrl = group.picture.mediumUri;
    }

    return doc;
};

/**
 * Given a user, produces a SearchDocument
 *
 * @param  {User}              user    The user for which to produce a search document
 * @return {SearchDocument}            A search document
 * @api private
 */
var _produceUserSearchDocument = function(user) {
    // Full text searches are done on the displayName and public alias of the user. We also sort by this content
    var fullText = _.compact([user.displayName, user.publicAlias]).join(' ');
    var doc = {
        'resourceType': user.resourceType,
        'id': user.id,
        'tenantAlias': user.tenant.alias,
        'displayName': user.displayName,
        'visibility': user.visibility,
        'q_high': fullText,
        'sort': fullText,
        '_extra': {
            'publicAlias': user.publicAlias,
            'userExtra': user.extra
        }
    };

    if (user.picture.mediumUri) {
        doc.thumbnailUrl = user.picture.mediumUri;
    }

    return doc;
};

/**
 * Produces a search document for access scoping 'group' resources
 *
 * @see SearchAPI#registerSearchDocumentProducer
 * @api private
 */
var _produceGroupMemberDocuments = function(data, callback) {
    AuthzAPI.getAuthzMembers(data.id, null, 10000, function(err, members) {
        if (err) {
            return callback(err);
        }

        var memberIds = _.map(members, function(member) { return member.id; });
        var childDoc = SearchUtil.createMembersDocument(data.id, memberIds);
        return callback(null, childDoc);
    });
};

/**
 * Produces a search document that contains all the groups a principal is a member of
 *
 * @see SearchAPI#registerSearchDocumentProducer
 * @api private
 */
var _producePrincipalMembershipsDocuments = function(data, callback) {
    // Get all the direct group memberships of the given resource
    //
    // Here we are filtering the memberships to "group" types. This means that we will only be able to search through group members.
    AuthzAPI.getRolesForPrincipalsAndResourceType([data.id], AuthzConstants.resourceTypes.GROUP, 10000, function(err, memberships) {
        if (err) {
            return callback(err);
        }

        var doc = {
            '_parent': data.id,
            '_type': PrincipalsConstants.search.resourceMembershipsMappingName,
            'id': data.id + '#memberships',
            'direct_memberships': _.keys(memberships)
        };
        return callback(null, doc);
    });
};


// Bind the document producers
SearchAPI.registerSearchDocumentProducer('user', _producePrincipalDocument);
SearchAPI.registerSearchDocumentProducer('group', _producePrincipalDocument);
SearchAPI.registerSearchDocumentProducer('group_members', _produceGroupMemberDocuments);
SearchAPI.registerSearchDocumentProducer('user_memberships', _producePrincipalMembershipsDocuments);
SearchAPI.registerSearchDocumentProducer('group_memberships', _producePrincipalMembershipsDocuments);



///////////////////////////
// DOCUMENT TRANSFORMERS //
///////////////////////////

/**
 * Given an array of user search documents, transform them into search documents suitable to be displayed to the user in context.
 *
 * @param  {Context}   ctx             The current context
 * @param  {Object}    docs            A hash, keyed by the document id, while the value is the document to transform
 * @param  {Function}  callback        Invoked when the process completes
 * @param  {Object}    callback.err    An error that occurred, if any
 * @param  {Object}    callback.docs   The transformed docs, in the same form as the `docs` parameter.
 * @api private
 */
var _transformUserDocuments = function(ctx, docs, callback) {
    var transformedDocs = {};
    var docIds = _.keys(docs);
    for (var i = 0; i < docIds.length; i++) {
        var docId = docIds[i];
        var doc = docs[docId];
        var fields = doc.fields;

        // First we need to convert the data in this document back into the source user object so that we may use PrincipalsUtil.hideUserData
        // to hide its information. We will then after convert the user *back* to a search document once the user information has been
        // scrubbed.
        var user = new User(fields.tenantAlias, docId, fields.displayName, {
            'visibility': fields.visibility,
            'publicAlias': fields._extra.publicAlias,
            'mediumPictureUri': fields.thumbnailUrl
        });
        user.extra = fields._extra.userExtra;

        // Hide information that is sensitive to the current session
        PrincipalsUtil.hideUserData(ctx, user);

        // Convert the user object back to a search document using the producer. We use this simply to re-use the logic of turning a user
        // object into a search document.
        var result = _produceUserSearchDocument(user);
        result.profilePath = user.profilePath;

        // Attach the compact tenant object on the search result.
        result.tenant = user.tenant;

        // The UI search model expects the 'extra' parameter if it was not scrubbed
        if (user.extra) {
            result.extra = user.extra;
        }

        // If the mediumPictureUri wasn't scrubbed from the user object that means the current user can see it.
        // Generate a downloadable url for it.
        if (user.picture.mediumUri) {
            result.thumbnailUrl = PrincipalsUtil.getSignedPictureUrl(ctx, ctx.tenant().alias, user.picture.mediumUri);
        }

        // We need to delete these fields which are added by the producer but aren't supposed to be included in the UI.
        delete result.q_high;
        delete result.sort;

        transformedDocs[docId] = result;
    }

    return callback(null, transformedDocs);
};

// Bind the transformer to the search API
SearchAPI.registerSearchDocumentTransformer('user', _transformUserDocuments);


/**
 * Given an array of group search documents, transform them into search documents suitable to be displayed to the user in context.
 *
 * @param  {Context}   ctx             The current context
 * @param  {Object}    docs            A hash, keyed by the document id, while the value is the document to transform
 * @param  {Function}  callback        Invoked when the process completes
 * @param  {Object}    callback.err    An error that occurred, if any
 * @param  {Object}    callback.docs   The transformed docs, in the same form as the `docs` parameter.
 * @api private
 */
var _transformGroupDocuments = function(ctx, docs, callback) {
    var transformedDocs = {};
    var docIds = _.keys(docs);
    for (var i = 0; i < docIds.length; i++) {
        var docId = docIds[i];
        var doc = docs[docId];
        var result = _.extend({}, doc.fields, {'id': doc['_id']});

        // Augment it with a downloadable thumbnail url if it has a thumbnail uri.
        // The thumbnail should not be hidden, even if the group is private.
        if (doc.fields.thumbnailUrl) {
            result.thumbnailUrl = PrincipalsUtil.getSignedPictureUrl(ctx, ctx.tenant().alias, doc.fields.thumbnailUrl);
        }

        if (doc.fields._extra && doc.fields._extra.alias) {
            result.alias = doc.fields._extra.alias;
        }

        // Attach the group tenant information
        result.tenant = TenantsAPI.getTenant(result.tenantAlias).compact();
        result.profilePath = '/group/' + result.tenant.alias + '/' + AuthzUtil.getResourceFromId(result.id).resourceId;

        transformedDocs[docId] = result;
    }

    return callback(null, transformedDocs);
};

// Bind the transformer to the search API
SearchAPI.registerSearchDocumentTransformer('group', _transformGroupDocuments);


/////////////////////////
// REINDEX ALL HANDLER //
/////////////////////////

/*!
 * Binds a reindexAll handler that reindexes all rows from the Principals CF (users and groups)
 */
SearchAPI.registerReindexAllHandler('principal', function(callback) {

    /*!
     * Handles each iteration of the PrincipalsDAO iterate all method, firing tasks for all principals to
     * be reindexed.
     *
     * @see PrincipalsDAO#iterateAll
     */
    var _onEach = function(principalRows, done) {
        // Aggregate group and user reindexing task resources
        var indexTasks = [];
        _.each(principalRows, function(principal) {
            var principalId = principal.principalId;
            if (principalId) {
                if (PrincipalsUtil.isGroup(principalId)) {
                    indexTasks.push(new IndexTask('group', {'id': principalId}));
                    indexTasks.push(new IndexTask('group_members', {'id': principalId}));
                    indexTasks.push(new IndexTask('group_memberships', {'id': principalId}));
                } else {
                    indexTasks.push(new IndexTask('user', {'id': principalId}));
                    indexTasks.push(new IndexTask('user_memberships', {'id': principalId}));
                }
            }
        });

        SearchAPI.submitIndexTasks(indexTasks, done);
    };

    PrincipalsDAO.iterateAll(null, 100, _onEach, callback);
});
