 ...
 * @api private
 */
var _transformUserDocuments = function(ctx, docs, callback, _transformedDocs) {
    var transformedDocs = {};
    _.each(docs, function(doc, docId) {
        ...
        // We use this simply to re-use the logic of turning a user object into
        // a search document
        var result = _produceUserSearchDocument(user);

        // Apply the necessarily information if it is still available after
        // being scrubbed with `hideUserdata`
        _.oaeExtendDefined(result, {
            'profilePath': user.profilePath,
            'tenant': user.tenant,
            'extra': user.extra,
            'thumbnailUrl': user.picture.mediumUri
        });

        // We need to delete these fields which are added by the producer but
        // aren't supposed to be included in the UI
    ...
    // Sign any thumbnail image urls
    _signThumbnailUrls(ctx, _.values(transformedDocs), function(err) {
        if (err) {
            return callback(err);
        }
...
/**
 * Sign the thumbnail urls on the given transformed search documents
 *
 * @param  {Context}    ctx                 Context of the current request
 * @param  {Object[]}   transformedDocs     The transformed search documents whose thumbnail uris to sign
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @api private
 */
var _signThumbnailUrls = function(ctx, transformedDocs, callback, _transformedDocs) {
    _transformedDocs = _transformedDocs || transformedDocs.slice();
    if (_.isEmpty(_transformedDocs)) {
        return callback();
    }

    var doc = _transformedDocs.shift();
    if (!doc.thumbnailUrl) {
        return setImmediate(_signThumbnailUrls, ctx, transformedDocs, callback, _transformedDocs);
    }

    ContentUtil.getSignedDownloadUrl(ctx, doc.thumbnailUrl, null, null, function(err, thumbnailUrl) {
        if (err) {
            return callback(err);
        }

        // Replace the thumbnail url with the signed version
        doc.thumbnailUrl = thumbnailUrl;

        return _signThumbnailUrls(ctx, transformedDocs, callback, _transformedDocs);
    });
};
