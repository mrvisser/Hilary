 ...
 */
ActivityAPI.registerActivityRouter('user', function(activityType, entityType, userResource, callback) {
    ...
    return callback(null, [userResource.resourceId]);
});

/*!
 * Routes a group entity when an activity has occurred.
 */
ActivityAPI.registerActivityRouter('group', function(activityType, entityType, groupResource, callback) {
    // Deliver to all the group members, direct or indirect
    ActivityUtil.getAllAuthzMembersByRole(groupResource.resourceId, function(err, membersByRole) {
        if (err) {
            return callback(err);
        }
