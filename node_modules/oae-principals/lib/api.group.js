 ...
 * @param  {String}    visibility           The visibility of the group. Should be one of `AuthzConstants.visibility`'s values
 * @param  {String}    joinable             How the group can be joined. Should be one of `AuthzConstants.joinable`'s values
...
var createGroup = module.exports.createGroup = function(ctx, displayName, description, visibility, joinable, roles, callback) {
    ...
    Config.getValues(tenantAlias, 'group', ['visibility', 'joinable'], function(err, groupConfig) {
        if (err) {
            return callback(err);
        }
        ...
        visibility = visibility || groupConfig.visibility;
        joinable = joinable || groupConfig.joinable;
        roles = roles || {};
