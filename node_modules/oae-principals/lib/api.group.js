...
var getFullGroupProfile = module.exports.getFullGroupProfile = function(ctx, groupId, callback) {
    getGroup(ctx, groupId, function(err, group) {
        ...
        AuthzAPI.isAllowed(ctx.user().id, Constants.roles.MANAGER, groupId, { probablyFalse: true }, function(err, isAllowed) {
            if (err) {
                return callback(err);
            }
            ...
            AuthzAPI.isAllowed(ctx.user().id, Constants.roles.MEMBER, groupId, { probablyFalse: true }, function(err, isAllowed) {
                if (err) {
                    return callback(err);
                }
