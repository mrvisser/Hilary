...
OAE.tenantServer.use(function(req, res, next) {
    ...
    // The Terms and Conditions middleware is only applicable on logged in users
    // who try to interact with the system excluding a set of whitelisted
    // endpoints
    var needsTermsAndConditionsCheck = (user && !_.contains(['GET', 'HEAD'], req.method) && !_isWhiteListed(req.path));
    OaeUtil.invokeIfNecessary(needsTermsAndConditionsCheck, PrincipalsAPI.needsToAcceptTermsAndConditions, ctx, function(err, needsToAccept) {
        if (err) {
