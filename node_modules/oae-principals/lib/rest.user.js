...
OAE.tenantRouter.on('get', '/api/user/termsAndConditions', function(req, res) {
    PrincipalsAPI.getTermsAndConditions(req.ctx, req.query.locale, function(err, termsAndConditions) {
        if (err) {
            return rest.send(err.code, err.msg);
        }

        return res.send(200, termsAndConditions);
    });
});
...
OAE.tenantRouter.on('post', '/api/user/create', function(req, res) {
    ...
    PrincipalsConfig.getValue(tenant.alias, 'termsAndConditions', 'enabled', function(err, needsTermsAndConditionsAgreement) {
        if (err) {
            return res.send(err.code, err.msg);
        } else if (needsTermsAndConditionsAgreement && opts.acceptedTC !== true) {
            return res.send(400, 'You need to accept the Terms and Conditions');
        }

        PrincipalsConfig.getValues(tenant.alias, 'recaptcha', ['enabled', 'publicKey', 'privateKey'], function(err, recaptchaConfig) {
            if (err) {
                return res.send(err.code, err.msg);
            } else if (!recaptchaConfig.enabled) {
                // If we don't need to pass recaptcha, just create the user
                return createUser();
            }
            ...
            var recaptchaPublicKey = recaptchaConfig.publicKey;
            var recaptchaPrivateKey = recaptchaConfig.privateKey;
