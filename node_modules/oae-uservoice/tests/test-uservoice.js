    ...
    describe('Profiles', function() {
        ...
        it('verify it does not transfer private or loggedin user information', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, mrvisser, nico, bert) {
                ...
                UservoiceProfile.createUservoiceProfile(publicUser, function(err, publicProfile) {
                    assert.ok(!err);
                    ...
                    UservoiceProfile.createUservoiceProfile(loggedinUser, function(err, loggedinProfile) {
                        assert.ok(!err);
                        ...
                        UservoiceProfile.createUservoiceProfile(privateUser, function(err, privateProfile) {
        ...
        /*!
         * Map a user profile to a uservocie profile and ensure the locale is
         * as expected
         *
         * @param  {Object}             profile         The user profile
         * @param  {String}             [userLocale]    The user profile locale to apply, if any
         * @param  {String}             expectedLocale  The expected uservoice locale
         * @param  {Function}           callback        Invoked when all assertions complete
         * @throws {AssertionError}                     Thrown if the locale is not as expected
         */
        var _assertUservoiceProfileLocaleEquals = function(profile, userLocale, expectedLocale, callback) {
            var profileWithLocale = _.extend({}, profile);
            if (userLocale) {
                profileWithLocale.locale = userLocale;
            }

            UservoiceProfile.createUservoiceProfile(profileWithLocale, function(err, uservoiceProfile) {
                assert.ok(!err);
                assert.strictEqual(uservoiceProfile.locale, expectedLocale);
                return callback();
            });
        };
        ...
        it('verify it properly maps the user locale to either a UserVoice-supported locale or nothing', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, mrvisser) {
                ...
                _assertUservoiceProfileLocaleEquals(mrvisser, 'nl-NL', 'nl', function() {
                    _assertUservoiceProfileLocaleEquals(mrvisser, 'nl_NL', 'nl', function() {

                        // Ensure an alternative locale mapping is used for the inconsistent representations in UserVoice
                        _assertUservoiceProfileLocaleEquals(mrvisser, 'fr-CA', 'nl', function() {
                            _assertUservoiceProfileLocaleEquals(mrvisser, 'fr_CA', 'nl', function() {

                                // Ensure a completely unsupported locale or no locale results in the tenant default being used
                                _assertUservoiceProfileLocaleEquals(mrvisser, null, 'en', function() {
                                    _assertUservoiceProfileLocaleEquals(mrvisser, 'af_ZA', 'en', function() {
                                        ...
                                        ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-principals/user/defaultLanguage': 'af_ZA'}, function(err) {
                                            ...
                                            // Ensure when mrvisser has an unknown or no locale, no locale is provided for the profile
                                            _assertUservoiceProfileLocaleEquals(mrvisser, 'af_ZA', undefined, function() {
                                                _assertUservoiceProfileLocaleEquals(mrvisser, null, undefined, function() {
                                                    return callback();
                                                });
                                            });
                                        });
