...
var getUrlInfo = module.exports.getUrlInfo = function(ctx, expiresIn, callback) {
    ...
    // Get the uservoice configuration values
    UservoiceConfig.getValues(ctx.tenant().alias, 'general', ['enabled', 'baseUrl', 'subdomain', 'ssoKey'], function(err, uservoiceConfig) {
        if (err) {
            return callback(err);
        }

        var enabled = uservoiceConfig.enabled;
        var baseUrl = uservoiceConfig.baseUrl;
        var subdomain = uservoiceConfig.subdomain;
        var ssoKey = uservoiceConfig.ssoKey;
...
var _generateAuthenticationToken = function(user, subdomain, ssoKey, expiresIn, callback) {
    UservoiceProfile.createUservoiceProfile(user, function(err, data) {
        if (err) {
            return callback(err);
        }
