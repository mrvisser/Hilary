...
var log = require('oae-logger').logger('helenus-logger-blah');
var Telemetry = require('oae-telemetry').telemetry('helenus');
...
CassandraClient.prototype.send_execute_cql_query = function(query, compression) {
  ...
  Telemetry.append('cql.query.send.length', query.length);
  Telemetry.append('cql.query.send.time', duration);
  ...
  if (query.length > 5000) {
    log().warn({
      ...
      time: duration,
      size: query.length
    }, 'Sending super long query to cassandra.');
  }
  */
...
//  var start = Date.now();
//  var size = input.trans.inBuf.length;
...
/*
  var duration = Date.now() - start;
  Telemetry.append('cql.query.rcv.length', size);
  Telemetry.append('cql.query.rcv.time', duration);

  if (size > 5000) {
    log().warn({
      size: size,
      time: duration,
      data, input.trans.inBuf.toString('utf8')
    }, 'Receiving super long response from cassandra.');
  }
*/
