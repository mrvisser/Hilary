...
var log = require('oae-logger').logger('helenus');
...
CassandraClient.prototype.send_execute_cql_query = function(query, compression) {
  ...
  if (query.length > 5000) {
    log().info({
      query: query.toString('utf8'),
      time: duration,
      size: query.length
    }, 'Long query to cassandra');
  }
...
CassandraClient.prototype.recv_execute_cql_query = function(input,mtype,rseqid) {
  ...
  if (size > 15000) {
    log().info({
      data: input.trans.infBuf.toString('utf8'),
      size: size,
      time: duration
    }, 'Long response from cassandra.');
  }
