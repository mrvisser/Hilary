...
var log = require('oae-logger').logger('cassandra_types');
var Telemetry = require('oae-telemetry').telemetry('cassandra_types');
...
CqlRow.prototype.read = function(input) {
  ...
  if (duration > 10) {
    log().info({
      err: new Error('Took a really long time to parse response column.'),
      i: i,
      time: duration,
      size: input.trans.inBuf.length,
      data: input.trans.inBuf.toString('binary')
    }, 'Read column: i=%s, time=%sms', i, duration);

    Telemetry.append('row.veryslowread.time', duration);
    Telemetry.append('row.veryslowread.size', input.trans.inBuf.length);
    Telemetry.incr('row.veryslowread.count');
  }
