 ...
 * @param  {String}         visibility              The visibility of the folder. One of `AuthzConstants.visibility`
        ...
        ContentDAO.Content.getMultipleContentItems(contentIds, null, function(err, contentItems) {
            ...
            var done = function() {
                FoldersContentLibrary.purge(folder, function(err) {
                    ...
                    ContentUtil.augmentContentItems(ctx, failedContent, null, null, function(err) {
                        if (err) {
                            return callback(err);
                        }
...
var getFolder = module.exports.getFolder = function(ctx, folderId, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        AuthzPermissions.canView(ctx, folder, function(err) {
            ...
            _augmentFolder(ctx, folder, function(err) {
                if (err) {
                    return callback(err);
                }
        ...
        LibraryAPI.Authz.resolveTargetLibraryAccess(ctx, principal.id, principal, function(err, hasAccess, visibility) {
            ...
            FoldersFoldersLibrary.list(principal, visibility, {'start': start, 'limit': limit}, function(err, folderIds, nextToken) {
                ...
                FoldersDAO.getFoldersByIds(folderIds, function(err, folders) {
                    ...
                    _augmentFolders(ctx, folders, function(err) {
                        if (err) {
                            return callback(err);
                        }
 ...
 * @api private
 */
var _augmentFolders = function(ctx, folders, callback, _folders) {
    _folders = _folders || folders.slice();
    if (_.isEmpty(_folders)) {
        return callback();
    }

    _augmentFolder(ctx, _folders.shift(), function(err) {
        if (err) {
            return callback(err);
        }

        return _augmentFolders(ctx, folders, callback, _folders);
    });
};
 ...
 * @api private
 */
var _augmentFolder = function(ctx, folder, callback) {
    var signedProperties = {
        'thumbnailUri': 'thumbnailUrl',
        'wideUri': 'wideUrl'
    };
    return ContentUtil.signObjectUrlProperties(ctx, [folder.previews], signedProperties, null, callback);
};
