...
var _folderTransformer = function(ctx, activityEntities, callback) {
    ...
    FoldersDAO.getFoldersByIds(folderIds, function(err, folders) {
        ...
        // Convert the nexted object of activity entity info into an array of
        // `entityInfos` so we can iterate over it deterministically
        ...
        // Transform all entities. The entity result will be in the same order
        // as they were given, so we map the transformed results back to the
        // entities by order
        _transformPersistentFolderActivityEntities(ctx, _.pluck(entityInfos, 'entity'), foldersById, function(err, transformedEntities) {
            if (err) {
                return callback(err);
            }

            // Apply all transformed entities to the result. We zip the entity
            // infos back with the transformed entities as they lay in the same
            // order
            _.chain(entityInfos)
                .zip(transformedEntities)
                .each(function(entityInfoTransformed) {
                    var entityInfo = entityInfoTransformed[0];
                    var transformedEntity = entityInfoTransformed[1];
                    transformedActivityEntities[entityInfo.activityId][entityInfo.entityId] = transformedEntity;
                })
                .value();
...
var _folderCommentTransformer = function(ctx, activityEntities, callback) {
    // Gather the entities into entity info objects that can be organized
    // into an array, transformed, and then brought back together to make
    // the transformed activity entities object
    var entityInfo = [];
    var transformedActivityEntities = {};
    ...
    MessageBoxUtil.transformPersistentMessageActivityEntities(ctx, _.pluck(entityInfo, 'entity'), function(err, transformedEntities) {
        if (err) {
            return callback(err);
        }

        // Zip the entity infos with the transformed entities as they will
        // be in the same order
        _.chain(entityInfo)
            .zip(transformedEntities)
            .each(function(entityInfoWithTransformedEntity) {
                var entityInfo = entityInfoWithTransformedEntity[0];
                var transformedEntity = entityInfoWithTransformedEntity[1];
                transformedActivityEntities[entityInfo.activityId][entityInfo.entityId] = transformedEntity;
            })
            .value();
...
var _folderCommentInternalTransformer = function(ctx, activityEntities, callback) {
    ...
    // the transformed activity entities object
    var entityInfo = [];
    var transformedInternalEntities = {};
    _.keys(activityEntities).forEach(function(activityId) {
        transformedInternalEntities[activityId] = transformedInternalEntities[activityId] || {};
        _.keys(activityEntities[activityId]).forEach(function(entityId) {
            var entity = activityEntities[activityId][entityId];
            transformedInternalEntities[activityId][entityId] = {};
            entityInfo.push({
                'activityId': activityId,
                'entityId': entityId,
    ...
    MessageBoxUtil.transformPersistentMessageActivityEntitiesToInternal(ctx, _.pluck(entityInfo, 'entity'), function(err, transformedEntities) {
        if (err) {
            return callback(err);
        }
        ...
        _.chain(entityInfo)
            ...
            .each(function(entityInfoWithTransformedEntity) {
                var entityInfo = entityInfoWithTransformedEntity[0];
                var transformedEntity = entityInfoWithTransformedEntity[1];
                transformedInternalEntities[entityInfo.activityId][entityInfo.entityId] = transformedEntity;
            })
            .value();

        return callback(null, transformedInternalEntities);
    });
};
...
/**
 * Transform an array of folder objects into activity entities suitable to be
 * displayed in an activity stream
 *
 ...
 * @param  {ActivityEntity[]}   callback.entities   The activity entities that represents the given folder entities
 * @api private
 */
var _transformPersistentFolderActivityEntities = function(ctx, entities, foldersById, callback, _entities, _transformedEntities) {
    ...
    _transformPersistentFolderActivityEntity(ctx, _entities.slice(), foldersById, function(err, transformedEntity) {
        if (err) {
            return callback(err);
        }

        _transformedEntities.push(transformedEntity);
        return _transformPersistentFolderActivityEntities(ctx, entities, foldersById, callback, _entities, _transformedEntities);
    });
};
 ...
 * @param  {Function}           callback            Standard callback function
 * @param  {Object}             callback.err        An error that occurred, if any
 * @param  {ActivityEntity}     callback.entity     The activity entity that represents the given folder entity
 * @api private
 */
var _transformPersistentFolderActivityEntity = function(ctx, entity, foldersById, callback) {
    ...
    // Sign the preview urls
    var signedProperties = {
        'thumbnailUri': 'thumbnailUrl',
        'wideUri': 'wideUrl'
    };
    ContentUtil.signObjectUrlProperties(ctx, [folder.previews], signedProperties, null, function(err) {
        if (err) {
            return callback(err);
        } else if (folder.previews) {
            if (folder.previews.thumbnailUrl) {
                opts.image = new ActivityModel.ActivityMediaLink(folder.previews.thumbnailUrl, PreviewConstants.SIZES.IMAGE.THUMBNAIL, PreviewConstants.SIZES.IMAGE.THUMBNAIL);
            }
            if (folder.previews.wideUrl) {
                opts.ext[ContentConstants.activity.PROP_OAE_WIDE_IMAGE] = new ActivityModel.ActivityMediaLink(folder.previews.wideUrl, PreviewConstants.SIZES.IMAGE.WIDE_WIDTH, PreviewConstants.SIZES.IMAGE.WIDE_HEIGHT);
            }
        }

        return callback(null, new ActivityModel.ActivityEntity('folder', globalId, folder.visibility, opts));
    });
};
