...
var sendEmail = module.exports.sendEmail = function(templateModule, templateId, recipient, data, opts, callback) {
    ...
    TenantsConfig.getValues(recipient.tenant.alias, 'instance', ['instanceName', 'instanceURL', 'hostingOrganization', 'hostingOrganizationURL'], function(err, instanceConfig) {
        if (err) {
            return callback(err);
        }
        ...
        var templateCtx = _.extend({}, data, {
            ...
            'instance': {
                ...
                'URL': instanceConfig.instanceUR
            },
            'hostingOrganization': {
                'name': instanceConfig.hostingOrganization,
                'URL': instanceConfig.hostingOrganizationUR
            }
        });
        ...
        // Get the tenant configuration for the email `from` information
        EmailConfig.getValues(recipient.tenant.alias, 'general', ['fromName', 'fromAddress'], function(err, generalEmailConfig) {
            if (err) {
                return callback(err);
            }
            ...
            var fromName = generalEmailConfig.fromName || tenant.displayName;
            fromName = fromName.replace('${tenant}', tenant.displayName);
            var fromAddr = generalEmailConfig.fromAddress || util.format('noreply@%s', tenant.host);
