...
var getContent = module.exports.getContent = function(ctx, contentId, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, contentItem) {
        if (err) {
            return callback(err);
        }

        AuthzPermissions.canView(ctx, contentItem, function(err) {
            if (err) {
                return callback(err);
            }

            ContentUtil.augmentContentItems(ctx, [contentItem], null, null, function(err) {
                if (err) {
                    return callback(err);
                }

                return callback(null, contentItem);
            });
        });
    });
};
...
var _createFile = function(ctx, displayName, description, visibility, file, additionalMembers, folders, callback) {
    ...
    ContentUtil.getStorageBackend(ctx, null, function(err, backend) {
        if (err) {
            return callback(err);
        }

        backend.store(ctx.tenant().alias, file, options, function(err, uri) {
            if (err) {
                return callback(err);
            }
...
var _canManage = function(ctx, contentId, callback) {
    ContentDAO.Content.getContent(contentId, function(err, content) {
        ...
        AuthzPermissions.canManage(ctx, content, function(err) {
            ...
            ContentUtil.augmentContentItems(ctx, [content], null, null, function(err) {
                if (err) {
                    return callback(err);
                }
...
var getSignedPreviewDownloadInfo = module.exports.getSignedPreviewDownloadInfo = function(ctx, contentId, revisionId, previewItem, signatureData, callback) {
    ...
    ContentDAO.Previews.getContentPreview(revisionId, previewItem, function(err, preview) {
        ...
        ContentUtil.getStorageBackend(ctx, preview.uri, function(err, backend) {
            if (err) {
                return callback(err);
            }

            backend.getDownloadStrategy(ctx.tenant().alias, preview.uri, function(err, downloadStrategy) {
                if (err) {
                    return callback(err);
                }
...
var getContentLibraryItems = module.exports.getContentLibraryItems = function(ctx, principalId, start, limit, callback) {
    ...
    PrincipalsDAO.getPrincipal(principalId, function(err, principal) {
        ...
        LibraryAPI.Authz.resolveTargetLibraryAccess(ctx, principal.id, principal, function(err, hasAccess, visibility) {
            ...
            ContentDAO.Content.getContentLibraryItems(principalId, visibility, start, limit, function(err, contentItems, nextToken) {
                if (err) {
                    return callback(err);
                }

                ContentUtil.augmentContentItems(ctx, contentItems, null, null, function(err) {
                    if (err) {
                        return callback(err);
                        }

                    // Emit an event indicating that the content library has been retrieved
                    ContentAPI.emit(ContentConstants.events.GET_CONTENT_LIBRARY, ctx, principalId, visibility, start, limit, contentItems);
...
var _getRevisions = function(ctx, contentObj, start, limit, opts, callback) {
    ...
    ContentDAO.Revisions.getRevisions(contentObj.id, start, limit, opts, function(err, revisions, nextToken) {
        ...
        PrincipalsUtil.getPrincipals(ctx, userIds, function(err, users) {
            ...
            // Sign the preview uris
            _augmentRevisions(ctx, revisions, function(err) {
                if (err) {
                    return callback(err);
                }
...
var getRevisionDownloadInfo = module.exports.getRevisionDownloadInfo = function(ctx, contentId, revisionId, callback) {
    ...
    getContent(ctx, contentId, function(err, content) {
        ...
        ContentDAO.Revisions.getRevision(revisionId, function(err, revision) {
            ...
            ContentUtil.getStorageBackend(ctx, revision.uri, function(err, backend) {
                if (err) {
                    return callback(err);
                }

                backend.getDownloadStrategy(ctx.tenant().alias, revision.uri, function(err, downloadStrategy) {
                    if (err) {
                        return callback(err);
                    }
...
var _getRevision = function(ctx, contentObj, revisionId, callback) {
    if (!revisionId) {
        ...
        ContentDAO.Revisions.getRevision(revisionId, function(err, revisionObj) {
            ...
            _augmentRevision(ctx, revisionObj, function(err) {
                if (err) {
                    return callback(err);
                }
...
/**
 * Convert the given revisions to a model that can be returned to the consumer from the API
 *
 * @param  {Context}        ctx             Standard context object containing the current user and the current tenant
 * @param  {Revision[]}     revisions       The revisions to augment
 * @param  {Function}       callback        Standard callback function
 * @param  {Object}         callback.err    An error that occurred, if any
 * @api private
 */
var _augmentRevisions = function(ctx, revisions, callback, _revisions) {
    _revisions = _revisions || revisions.slice();
    if (_.isEmpty(_revisions)) {
        return callback(null, _revisions);
    }

    _augmentRevision(ctx, _revisions.shift(), function(err) {
        if (err) {
            return callback(err);
        }

        return _augmentRevisions(ctx, revisions, callback, _revisions);
    });
};
 ...
 * @api private
 */
var _augmentRevision = function(ctx, revision, callback) {
    var signedProperties = {
        'thumbnailUri': 'thumbnailUrl',
        'mediumUri': 'mediumUrl'
    };
    return ContentUtil.signObjectUrlProperties(ctx, [revision], null, callback);
};
        ...
        ContentDAO.Revisions.getRevision(revisionId, function(err, revision) {
            ...
            ContentDAO.Revisions.createRevision(newRevisionId, contentId, ctx.user().id, revision, function(err, newRevision) {
                ...
                ContentDAO.Content.updateContent(contentObj, updates, true, function(err, newContentObj) {
                    ...
                    _augmentRevision(ctx, newRevision, function(err) {
                        if (err) {
                            return callback(err);
                        }
...
var verifySignedDownloadQueryString = module.exports.verifySignedDownloadQueryString = function(ctx, qs, callback) {
    ...
    ContentUtil.getStorageBackend(ctx, uri, function(err, backend) {
        if (err) {
            return callback(err);
        }
...
var _storePreview = function(ctx, previewReference, options, callback) {
    if (_.isString(previewReference)) {
        ...
        // to reference it
        return setImmediate(callback, null, 'remote:' + previewReference);
    }
    ...
    ContentUtil.getStorageBackend(ctx, null, function(err, backend) {
        ...
        return backend.store(ctx.tenant().alias, previewReference, options, callback);
    });
};
