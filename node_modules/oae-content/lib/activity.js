...
ContentAPI.on(ContentConstants.events.CREATED_CONTENT, function(ctx, content, revision, members) {
    ...
    // Iterate all members of the content item. We add the current user in as a member of the content item
    // as well
    _.chain(members).keys().union(ctx.user().id).map(_createMemberActivitySeedResource).map(function(principalResource) {
        ActivityAPI.postActivity(ctx, new ActivityModel.ActivitySeed(ContentConstants.activity.ACTIVITY_CONTENT_CREATE, millis, ActivityConstants.verbs.CREATE, actorResource, objectResource));
    });
});
...
ContentAPI.on(ContentConstants.events.UPDATED_CONTENT_MEMBERS, function(ctx, content, permissionChanges, newMemberIds, updatedMemberIds) {
    ...
    if (_.contains(newMemberIds, ctx.user().id)) {
        ...
        // Remove the actor user themself from the array as the rest will get share activities
        newMemberIds = _.without(newMemberIds, ctx.user().id);
    }

    // For any other user that was added as a member, fire a share activity
    _.chain(newMemberIds).map(_createMemberActivitySeedResource).each(function(principalResource) {
    ...
    _.chain(updatedMemberIds).map(_createMemberActivitySeedResource).each(function(principalResource) {
...
/**
 * Given a principal id, create an activity seed resource that represents them as either their User
 * or Group entity
 *
 * @param  {String}               principalId   The ids of the members to convert into activity seed resources
 * @return {ActivitySeedResource}               The activity seed resources that represent the principal
 * @api private
 */
var _createMemberActivitySeedResource = function(principalId) {
    var principalResourceType = (PrincipalsUtil.isGroup(principalId)) ? 'group' : 'user';
    return new ActivityModel.ActivitySeedResource(principalResourceType, principalId);
};
