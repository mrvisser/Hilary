...
var _contentTransformer = function(ctx, activityEntities, callback) {
    ...
    ContentDAO.Previews.getPreviewUris(allRevisionIds, function(err, previewsByLatestRevisionId) {
        if (err) {
            return callback(err);
        }
        ...
        _.each(activityEntities, function(entities, activityId) {
            ...
            _.each(entities, function(entity, entityId) {
                // Seed the transformed activity entities object as well as
                // build the entity info array
        ...
        // Transform all the entities
        ContentUtil.transformPersistentContentActivityEntities(ctx, _.pluck(entityInfo, 'entity'), previewsByLatestRevisionId, function(err, transformedEntities) {
            if (err) {
                return callback(err);
            }
...
var _contentInternalTransformer = function(ctx, activityEntities, callback) {
    ...
    ContentDAO.Revisions.getMultipleRevisions(allRevisionIds, {'fields': ['revisionId', 'previews']}, function(err, revisions) {
        ...
        var previewsByLatestRevisionId = {};
        _.each(revisions, function(revision) {
            previewsByLatestRevisionId[revision.revisionId] = revision.previews;
        });

        // Gather the entities into entity info objects that can be organized
        // into an array, transformed, and then brought back together to make
        // the transformed internal entities object
        var entityInfo = [];
        var transformedInternalEntities = {};
        _.each(activityEntities, function(entities, activityId) {
            transformedInternalEntities[activityId] = {};
            _.each(entities, function(entity, entityId) {
                // Seed the transformed internal entities object as well as
                // build the entity info array
        ...
        ContentUtil.transformPersistentContentActivityEntitiesToInternal(ctx, _.pluck(entityInfo, 'entity'), previewsByLatestRevisionId, function(err, transformedEntities) {
            if (err) {
                return callback(err);
            }
