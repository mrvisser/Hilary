...
ContentAPI.on(ContentConstants.events.CREATED_COMMENT, function(ctx, comment) {
    var millis = (new Date()).getTime();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id);
    var objectResource = new ActivityModel.ActivitySeedResource('content-comment', comment.commentId, {'contentId': comment.contentId});
 ...
 */
ActivityAPI.registerActivityEntityProducer('content', function(activityType, entityType, resources, callback) {
 ...
 */
ActivityAPI.registerActivityEntityProducer('content-comment', function(activityType, entityType, resources, callback) {
    // TODO: When _getContent is no longer exposed on ContentAPI, there should be something like a ContentDAO.getContent available to do get content w/o permission checks instead
    var resource = resources[0];
 ...
 */
ActivityAPI.registerActivityRouter('content', function(activityType, entityType, contentResource, callback) {
    // Extract the OAE ID from the activity object to get the content id
    return _routeContent(activityType, entityType, contentResource.resourceId, callback);
});

/*!
 * Routes an activity that occurred for a comment activity entity
 */
ActivityAPI.registerActivityRouter('content-comment', function(activityType, entityType, commentResource, callback) {
    // Extract the parent content id from the activity object, as the routes are identical
    return _routeContent(activityType, entityType, commentResource.resourceData.comment.contentId, callback);
});
