...
var index = module.exports.index = function(typeName, id, doc, options, callback) {
    ...
    _exec('index', client.index(index, typeName, doc, id, options), callback);
};
...
var bulk = module.exports.bulk = function(operations, callback) {
    ...
    _exec('bulk', client.bulk(operations, null), function(err, data) {
        if (err) {
            return callback(err);
        }

        /*!
         * The bulk response will look something like this:
         *
         * ```javascript
         *  {
         *      "took": 14,
         ...
         * ```
         *
         * In order to determine if there were any errors, we first check the `errors`
         * property. If `true`, we gather the document metadata items that failed.
         */
        if (data.errors) {
            err = {
                'code': 500,
                'msg': 'An unexpected error occurred bulk indexing a number of items',
                'errors': _.filter(data.items, function(item) {
                    return (_.chain(item).values().first().value().status !== 200);
                })
            };
        }
