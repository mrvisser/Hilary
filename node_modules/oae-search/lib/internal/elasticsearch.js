...
var numberOfIndexOperationsTried = {};
var numberOfIndexOperationsPassed = {};
...
var refreshSearchConfiguration = module.exports.refreshSearchConfiguration = function(_index, serverOpts) {
    ...
    // Every 10 seconds, output what we've indexed
    clearInterval(interval);
    interval = setInterval(function() {
        log().info({
            'tried': numberOfIndexOperationsTried,
            'passed': numberOfIndexOperationsPassed
        }, 'Indexing status');
    }, 10000);
};
...
var index = module.exports.index = function(typeName, id, doc, options, callback) {
    if (doc.resourceType) {
        numberOfIndexOperationsTried[doc.resourceType] = numberOfIndexOperationsTried[doc.resourceType] || 0;
        numberOfIndexOperationsTried[doc.resourceType]++;
    }

    log().trace({ 'typeName': typeName, 'id': id, 'document': doc, 'options': options }, 'Indexing a document');
    _exec('index', client.index(index, typeName, doc, id, options), function(err) {
        if (err) {
            return callback(err);
        }

        if (doc.resourceType) {
            numberOfIndexOperationsPassed[doc.resourceType] = numberOfIndexOperationsPassed[doc.resourceType] || 0;
            numberOfIndexOperationsPassed[doc.resourceType]++;
        }
...
var bulk = module.exports.bulk = function(operations, callback) {
    ...
    for (var i = 0; i < operations.length; i++) {
        ...
        if (keys.length === 1 && (meta['create'] || meta['index'] || meta['delete'])) {
            ...
            log().info(meta, 'not meta');
            if (meta.resourceType) {
                // It is a document line, maintain our counts
                currentOperationCount[meta.resourceType] = currentOperationCount[meta.resourceType] || 0;
                currentOperationCount[meta.resourceType]++;
                numberOfIndexOperationsTried[meta.resourceType] = numberOfIndexOperationsTried[meta.resourceType] || 0;
                numberOfIndexOperationsTried[meta.resourceType]++;
            }
        }
    }

    log().trace({'operations': operations}, 'Performing a bulk set of %s operations.', numOps);
    return _exec('bulk', client.bulk(operations, null), function(err) {
        if (err) {
            return callback(err);
        }

        // Maintain our passed index operation count
        _.each(currentOperationCount, function(count, resourceType) {
            numberOfIndexOperationsPassed[resourceType] = numberOfIndexOperationsPassed[resourceType] || 0;
            numberOfIndexOperationsPassed[resourceType] += count;
        });
