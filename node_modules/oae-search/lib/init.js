...
var ensureSearchSchema = function(config, callback) {
    ...
    SearchAPI.putMapping(SearchConstants.resourceMappingName, resourceSchema, {'_source': false}, function(err) {
        if (err) {
            return callback(err);
        }

        // The members type will be a child of the resource type
        var childOpts = {
            '_source': false,
            '_parent': SearchConstants.resourceMappingName
        };

        SearchAPI.putMapping(SearchConstants.resourceMembersMappingName, resourceMembersSchema, childOpts, callback);
    });
};
