...
module.exports = function(ctx, opts, callback) {
    ...
    opts.scope = SearchUtil.getScopeParam(opts.scope, ctx.tenant().alias);
    ...
    if (!needsFilterByAccess) {
        // No need to apply any access filter, so we can skip directly to generating the search
        return _search(ctx, opts, callback);
    } else if (!opts.includeIndirect) {
        // We only need to search by direct membership, so include those who have the current user
        // in their members list
        opts.access = [ctx.user().id];
        return _search(ctx, opts, callback);
    }

    // We are including access filters and we are including indirect group membership, so we'll need to know the group membership
    // of this user to scope by resources they have access to directly or indirectly
...
var _search = function(ctx, opts, callback) {
    ...
    var scopeFilter = (opts.scope !== SearchConstants.scope.ALL) ? SearchUtil.filterTerm('tenantAlias', opts.scope) : null;
    ...
    if (ctx.user() && ctx.user().isGlobalAdmin()) {
        // As the user is a global admin, we get everything from the specified tenant scope
        filter = baseFilter;
    } else if (ctx.user()) {
        ...
        filter = SearchUtil.filterAnd(
            ...
            SearchUtil.filterOr(
                ...
                // I can always see content that I have explicit access to, even if it belongs to another tenant and I'm limiting my
                // search to some "scope"
                accessFilter,

                SearchUtil.filterAnd(

                    // If I don't have explicit access to the resource, it should only show up in the items within the scope I've
                    // specified
                    scopeFilter,

                // Public resources from the current tenant and external permeable tenants can always be seen
                SearchUtil.filterAnd(
                ...
                // I can see all joinable resources from my own tenant and tenants that I can interact with
                SearchUtil.filterAnd(
                    SearchUtil.filterOr(
                        SearchUtil.filterTerm('tenantAlias', ctx.tenant().alias),
                        (interactingTenantAliases.length > 0) ? SearchUtil.filterTerms('tenantAlias', interactingTenantAliases) : null
