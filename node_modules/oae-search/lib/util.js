 ...
 * @param  {Function}   callback                            Standard callback function
 * @param  {Object}     callback.err                        An error that occurred, if any
 * @param  {Object}     callback.interactingTenantsFilter   The filter that narrows results down to only tenants with which the specified tenant alias
 */
var filterInteractingTenants = module.exports.filterInteractingTenants = function(tenantAlias, callback) {
    TenantsUtil.isPrivate(tenantAlias, function(err, isPrivate) {
        if (err) {
            return callback(err);
        }

        // Determine the filter to use based on the privacy status
        var filter = null;
        if (isPrivate) {
            filter = filterTerm('tenantAlias', tenantAlias);
        } else {
            ...
            filter = filterNot(filterTerms('tenantAlias', nonInteractingTenantAliases));
        }
...
var filterScopeAndAccess = module.exports.filterScopeAndAccess = function(ctx, scope, needsFilterByExplicitAccess, callback) {
    ...
    // Create a search filter that limits to only tenants that the current
    // tenant can interact with, if any
    filterInteractingTenants(tenant.alias, function(err, interactingTenantAliasesFilter) {
        if (err) {
            return callback(err);
        }
