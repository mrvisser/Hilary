...
/**
 * Determine if the given parameter is a valid scope parameter. If so, simply return the `val`, otherwise, return `defaultVal`.
 *
 * @param  {String}     val             The value to check
 * @param  {String}     [defaultVal]    The value to return if `val` is not valid. Defaults to `SearchConstants.scope.ALL`
 * @return {String}                     `val` if it is valid. `defaultVal` otherwise
 */
var getScopeParam = module.exports.getScopeParam = function(val, defaultVal) {
    // Simply do a falsey check to determine if the value is valid. We could potentially check the list of known tenant aliases
    // and provide _all if not valid, however it's not clear that it would actually be desired behaviour. Instead, we'll just
    // return a non-existing scope
    defaultVal = defaultVal || SearchConstants.scope.ALL;
    return val || defaultVal;
};
