 ...
 * @param  {Number}     maxConcurrentIndexers   The maximum number of indexing jobs that can be handled by the server at one time
 * @param  {Function}   callback                Invoked when configuration has been refreshed
 */
var refreshSearchConfiguration = module.exports.refreshSearchConfiguration = function(indexName, serverOpts, processIndexJobs, maxConcurrentIndexers, callback) {
    client.refreshSearchConfiguration(indexName, serverOpts);

    if (processIndexJobs && !boundIndexWorkers) {
        boundIndexWorkers = true;
        MQ.bind(SearchConstants.mq.TASK_INDEX_DOCUMENT, _handleIndexDocumentTask, {'subscribe': {'prefetchCount': maxConcurrentIndexers}}, function() {
