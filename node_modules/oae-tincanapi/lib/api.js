...
var sendStatementsToLRS = module.exports.sendActivitiesToLRS = function(statements, tenantAlias) {
    ...
    // We perform this first in a separate request since in most cases tincan
    // will be disabled, so we will only cache all configuration for those that
    // use it and not thousands of tenancies
    TinCanConfig.getValue(tenantAlias, 'lrs', 'enabled', function(err, enabled) {
        if (err) {
            log().error({
                'err', err,
                'tenantAlias': tenantAlias
            }, 'An error occurred while checking if tin can is enabled for tenant');
            return;
        } else if (!enabled) {
            return;
        }

        TinCanConfig.getValues(tenantAlias, 'lrs', ['endpoint', 'username', 'password'], function(err, tinCanConfig) {
            if (err) {
                log().error({
                    'err', err,
                    'tenantAlias': tenantAlias
                }, 'An error occurred while tin can configuration');
                return;
            }
            ...
            var options = {
                'method': 'POST',
                'url': tinCanConfig.endpoint,
                'timeout': config.timeout,
                'auth': {
                    'user': tinCanConfig.username,
                    'pass': tinCanConfig.password,
