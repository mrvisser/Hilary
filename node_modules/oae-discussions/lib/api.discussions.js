...
var createDiscussion = module.exports.createDiscussion = function(ctx, displayName, description, visibility, roles, opts, callback) {
    ...
    _resolveVisibility(ctx, visibility, function(err, visibility) {
        if (err) {
            return callback(err);
        }
...
/**
 * Get the discussion visibility from the given user-specified one, if any. Note
 * that the resulting visibility should still be validated as user-specified
 * visibilities are taken as-is
 *
 * @param  {Context}    ctx                     Context of the current request
 * @param  {String}     [visibility]            The specified visibility, if any
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred, if any
 * @param  {String}     callback.visibility     The visibility to use for creating the discussion
 * @api private
 */
var _resolveVisibility = function(ctx, visibility, callback) {
    if (visibility) {
        return callback(null, visibility);
    }

    return DiscussionsConfig.getValue(ctx.tenant().alias, 'visibility', 'discussion', callback);
};
