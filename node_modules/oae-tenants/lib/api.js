...
var _cacheTenants = function(callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "Tenant"', false, function(err, rows) {
        ...
        // Require this inline to avoid cross dependency
        var TenantConfig = require('oae-config').config('oae-tenants');
        TenantConfig.getAllValues('tenantprivacy', 'tenantprivate', function(err, result) {
            if (err) {
                return callback(err);
            }

            // Create the index of non-interactable tenants based on the default
            // value and overrides
            if (result.defaultValue === false) {
                _.each(result.overrides, function(value, tenantAlias) {
                    if (value === true) {
                        tenantsNotInteractable[tenantAlias] = tenants[tenantAlias];
                    }
                });
            } else if (result.defaultValue === true) {
                tenantsNotInteractable = _.extend({}, tenants);
                _.each(result.overrides, function(value, tenantAlias) {
                    if (value === false) {
                        delete tenantsNotInteractable[tenantAlias];
                    }
                });
            }
