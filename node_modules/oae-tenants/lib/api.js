...
var _cacheTenants = function(callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "Tenant"', false, function(err, rows) {
        ...
        _filterToPrivate(_.keys(tenants), function(err, privateTenantAliases) {
            if (err) {
                return callback(err);
            }

            _.each(privateTenantAliases, function(privateTenantAlias) {
                tenantsNotInteractable[privateTenantAlias] = tenants[privateTenantAlias];
            });
...
/**
 * Given a list of tenant aliases, get a list of them that reference private
 * tenants (i.e., tenants that are configured to be private)
 *
 * @param  {String[]}   tenantAliases                   The tenant aliases to filter
 * @param  {Function}   callback                        Standard callback function
 * @param  {Object}     callback.err                    An error that occurred, if any
 * @param  {String[]}   callback.privateTenantAliases   The tenant aliases that reference private tenants
 * @api private
 */
var _filterToPrivate = function(tenantAliases, callback, _tenantAliases, _private) {
    _tenantAliases = _tenantAliases || tenantAliases.slice();
    _private = _private || [];
    if (_.isEmpty(_tenantAliases)) {
        return callback(null, _private);
    }

    var tenantAlias = _tenantAliases.shift();

    // Require tenant config inline to avoid cross-dependency issues
    require('oae-config').config('oae-tenants').getValue(tenantAlias, 'tenantprivacy', 'tenantprivate', function(err, isPrivate) {
        if (err) {
            return callback(err);
        } else if (isPrivate) {
            _private.push(tenantAlias);
        }

        return _filterToPrivate(tenantAliases, callback, _tenantAliases, _private);
    });
};
...
var _updateCachedTenant = function(tenantAlias, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "Tenant" WHERE "alias" = ?', [tenantAlias], function(err, rows) {
        ...
        TenantsConfig.getValue(tenant.alias, 'tenantprivacy', 'tenantprivate', function(err, isPrivate) {
            if (err) {
                return callback(err);
            }
            ...
            if (tenant.isGlobalAdminServer || !tenant.active || tenant.deleted || isPrivate) {
...
/**
 * Determine if the given tenant alias is private
 *
 * @param  {String}     tenantAlias     The alias of the tenant to check
 * @return {Tenant}                     Will be the private tenant if it is private. If the tenant is not private, then this will return `false`y
 */
var isPrivate = module.exports.isPrivate = function(tenantAlias) {
    return tenantsNotInteractable[tenantAlias];
};
 ...
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {Object[]}   callback.blocks     The configured landing page blocks for the current tenant
 */
var getLandingPage = module.exports.getLandingPage = function(ctx, callback) {
    // Ensure we only call the callback once between the error scenario and the
    // success scenario
    ...
    // Invokes the callback after each block has finished
    var _success = _.after(numBlocks, function() {
        return callback(null, landingPage);
    });

    for (var i = 1; i <= numBlocks + 1; i++) {
        var blockName = util.format('block_%d', i);
        _getLandingPageBlock(ctx, blockName, function(err, block) {
            if (err) {
                return callback(err);
            }
 ...
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occurred, if any
 * @param  {Object}     callback.block  The landing page block
 * @api private
 */
var _getLandingPageBlock = function(ctx, blockName, callback) {
    var block = {};
    var attributeNames = [
        'type',
        'xs',
    ...
    // Get the tenant config values for the landing page
    var TenantsConfig = require('oae-config').config('oae-tenants');
    TenantsConfig.getValue(ctx.tenant().alias, blockName, attributeNames, function(err, block) {
        if (err) {
            return callback(err);
        }
        ...
        _.oaeExtendDefined(block, {
            'imgUrl': UIAPI.getHashedPath(block.imgUrl),
            'videoPlaceholder': UIAPI.getHashedPath(block.videoPlaceholder)
        });
