 ...
 * @param  {Boolean}    callback.isPrivate  `true` if the tenant is private. `false` otherwise
 */
var isPrivate = module.exports.isPrivate = function(tenantAlias, callback) {
    TenantsConfig.getValue(tenantAlias, 'tenantprivacy', 'tenantprivate', function(err, isPrivate) {
        if (err) {
            return callback(err);
        }

        return callback(null, (isPrivate !== false));
    });
};
 ...
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred, if any
 * @param  {Boolean}    callback.canInteract    `true` if the tenant boundaries should allow these two tenants to interact with one another. `false` otherwise
 * @return {Boolean}
 */
var canInteract = module.exports.canInteract = function(tenantAliasOne, tenantAliasOther, callback) {
    ...
    isPrivate(tenatnAliasOne, function(err, tenantAliasOnePrivate) {
        if (err) {
            return callback(err);
        } else if (tenantAliasOnePrivate) {
            return callback(null, false);
        }

        isPrivate(tenantAliasOther, function(err, tenantAliasOtherPrivate) {
            if (err) {
                return callback(err);
            } else if (tenantAliasOtherPrivate) {
                return callback(null, false);
            }

            // Neither tenant is private, so they can interact with eachother
