...
var test = module.exports.test = function(ctx, content, callback) {
    ...
    } else if (!_isVide(ctx.revision.mime) && !_isAudio(ctx.revision.mime)) {
        return callback(null, -1);
    }

    MediaCoreConfig.getValue(content.tenant.alias, 'mediacore', 'enabled', function(err, isEnabled) {
        if (err) {
            return callback(err);
        } else if (!isEnabled) {
            return callback(null, -1);
...
var generatePreviews = module.exports.generatePreviews = function(ctx, content, callback) {
    ...
    ctx.download(function(err, path) {
        ...
        MediaCoreUtil.getConfig(tenantAlias, function(err, mediaCoreConfig) {
            if (err) {
                return callback(err);
            }
