...
var getPrincipalMemberships = module.exports.getPrincipalMemberships = function(principalId, start, limit, callback) {
    ...
    // We select one more to account for the slug _ column in the cache
    limit++;
    ...
    Cassandra.runPagedColumnQuery('AuthzMembershipsCache', 'principalId', principalId, start, limit, null, function(err, row, startMatched) {
        ...
        var memberships = [];
        // If this is a valid cache, we will at *least* select a slug _ column
        if (row.count > 0 || startMatched) {
            // Build the memberships list, ommitting the slug column if it was part of the range.
            var hadSlug = false;
            ...
            if (!hadSlug) {
                // If we didn't hit the slug, we selected one too many. Chop one off the top
                memberships.pop();
            }

            return callback(null, memberships);
        } else {
            // If we missed the cache, decrement the limit back down since we don't have to account for a slug column anymore
            limit--;
