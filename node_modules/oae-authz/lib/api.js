...
var log = require('oae-logger').logger('authz');
...
var getRolesForPrincipalsAndResourceType = module.exports.getRolesForPrincipalsAndResourceType = function(principalUuids, resourceType, limit, callback) {
    ...
    _getRolesForPrincipalsAndResourceType(principalUuids, resourceType, limit, callback);
};

/**
 * @see getRolesForPrincipalsAndResourceType
 */
var _getRolesForPrincipalsAndResourceType = function(principalUuids, resourceType, limit, callback) {
    if (resourceType) {
        ...
        var params = [start, end, principalUuids];
    } else {
        // if no resourceType is specified, grab all resources
        var cql = 'SELECT FIRST ' + limit + ' * FROM AuthzRoles USING CONSISTENCY QUORUM WHERE principalId IN (?)';
        var params = [principalUuids];
    }
...
var _applyRoleChanges = function(resourceUuid, changes, callback) {
    ...
    // Variables for bloom updates
    //  1. If a user's role is updated on a resource, push that value into the bloom for the user, resource, role
    //  2. If a group's role is updated on a resource, push that value into the bloom for all its user descendants
    var usersBloomUpdate = {};
    var groupsBloomUpdate = {};
    var groupRoles = {};

    for (var i = 0, keys = _.keys(changes); i < keys.length; i++) {
        var principalUuid = keys[i];

        var isGroup = AuthzUtil.isGroupPrincipal(principalUuid);
        ...
        if (changes[principalUuid]) {
            ...
            if (isGroup) {
                groupsBloomUpdate[principalUuid] = changes[principalUuid];
                groupRoles[changes[principalUuid]] = true;
            } else {
                if (!usersBloomUpdate[principalUuid]) {
                    usersBloomUpdate[principalUuid] = [];
                }

                usersBloomUpdate[principalUuid].push({
                    resourceUuid: resourceUuid,
                    role: changes[principalUuid]
                });
            }
    ...
    // bloomGroups and invalidGroups are both invalidated, but we diff with invalidGroups to avoid fetching their descendants more
    // than once
    var bloomGroups = _.keys(groupsBloomUpdate);
    var invalidGroups = _.difference(_.keys(groupsToInvalidate), bloomGroups);
    groupRoles = _.keys(groupRoles);

    // first get all the descendants of the bloom-update groups
    _getAllGroupMemberDescendants(bloomGroups, function(err, bloomMemberUuids) {
        if (err) {
            return callback(err);
        }

        // get all the descendants of the invalidated groups
        _getAllGroupMemberDescendants(_.keys(groupsToInvalidate), function(err, invalidMemberUuids) {
            if (err) {
                return callback(err);
            }

            // process the bloom descendants
            flattenValues(bloomMemberUuids).forEach(function(memberUuid) {
                // update the bloom for users only
                if (AuthzUtil.isUserPrincipal(memberUuid)) {
                    // add the roles to the bloom for this principal
                    groupRoles.forEach(function(role) {
                        if (!usersBloomUpdate[memberUuid]) {
                            usersBloomUpdate[memberUuid] = [];
                        }

                        usersBloomUpdate[memberUuid].push({
                            resourceUuid: resourceUuid,
                            role: role
                        });
                    });
                ...
                // invalidate all these principals
                principalsToInvalidate[memberUuid] = true;
            });

            // process the invalidation descendants
            flattenValues(invalidMemberUuids).forEach(function(memberUuid) {
                principalsToInvalidate[memberUuid] = true;
            });


            // update the bloom first since it will provide a hard response on "isManage" type queries
            _bloomUpdateUsersAccess(usersBloomUpdate, function(err) {
                ...
                Cassandra.runBatchQuery(queries, 'QUORUM', function(err) {
                    ...
                    // do this after the queries are run above to avoid race conditions
 ...
 * @param {Object}                      callback.err            An error that occurred, if any
 * @param {Object}                      callback.members        A hash keyed by the groupUuid (as provided), with value set to an array of principals that are a member
 *                                                              of the group
 */
var _getAllGroupMemberDescendants = function(groupUuids, callback) {
    ...
    var _multiGetAllGroupMemberDescendants = function(groupUuids, result, callback) {
        _getGroupMembersForGroups(groupUuids, function(err, memberUuids) {
            ...
            if (nextGroupUuidBatch.length > 0) {
                return _multiGetAllGroupMemberDescendants(nextGroupUuidBatch, result, callback);
            } else {
    ...
    var groupMembersHash = {};
    var resultErr = null;
    var done = 0;

    groupUuids.forEach(function(groupUuid) {
        // split the processes by all requested groups so we can see white groups had which descendants
        _multiGetAllGroupMemberDescendants(groupUuid, {}, function(err, memberUuids) {
            if (resultErr) {
                // do nothing, we've already errored
            } else if (err) {
                resultErr = err;
                return callback(resultErr);
            } else {
                groupMembersHash[groupUuid] = memberUuids;

                done++;
                if (done >= groupUuids.length) {
                    return callback(null, groupMembersHash);
                }
            }
        }); 
    });
...
var _applyGroupMembershipChanges = function(groupUuid, changes, callback) {
    ...
    // variables for tracking bloom filter updates.
    //  1.  If a user's role is set / updated in a group
    //          * add an entry for the given group to the user's bloom
    //          * add all roles the parent group has on all resources to the user's bloom
    //  2.  If a group's role is set / updated in a group
    //          * add an entry for the given group to all descendant user's blooms
    //          * add an entry for all resources accessible by the group to all descendant user's blooms
    //
    var bloomUpdates = {};
    var usersToUpdateForBloom = {};
    var groupRoles = {};
    var parentResourceRoles = [];
    ...
    _.keys(changes).forEach(function(memberUuid) {
        ...
        if (role === false) {
            ...
            if (AuthzUtil.isGroupPrincipal(memberUuid)) {
                // these are actually the same as the updated groups for bloom filters
                groupsToInvalidateForResource[memberUuid] = true;

                // know which group is getting what role so we can assign it to descendant blooms
                if (!groupRoles[memberUuid]) {
                    groupRoles[memberUuid] = [];
                }
                groupRoles[memberUuid].push(role);

            } else {
                // this user's bloom should be updated for this direct group association
                if (!bloomUpdates[memberUuid]) {
                    bloomUpdates[memberUuid] = [];
                }
                usersToUpdateForBloom[memberUuid] = true;
                bloomUpdates[memberUuid].push({
    ...
    _getAllGroupMemberDescendants(_.keys(groupsToInvalidate), function(err, descendants) {
        if (err) {
            return callback(err);
        }

        // aggregate COMPLETE cache invalidations for each descendant
        flattenValues(descendants).forEach(function(descendantUuid) {
            principalsToInvalidate[descendantUuid] = true; 
        });

        _getAllGroupMemberDescendants(_.keys(groupsToInvalidateForResource), function(err, descendants) {
            if (err) {
                return callback(err);
            }

            // Aggregate the following:
            //  A. Principals whose cache entry for the target group should be invalidated
            //  B. Bloom update entries for descendants of the added groups
            //  C. Users whose bloom must be updated for the resource access closure of the parent group
            //
            for (var groupToInvalidateForResourceUuid in descendants) {
                var descendantUuids = descendants[groupToInvalidateForResourceUuid];
                descendantUuids.forEach(function(descendantUuid) {

                    // A.
                    principalsToInvalidateForResource[descendantUuid] = true;

                    if (AuthzUtil.isUserPrincipal(descendantUuid)) {
                        // B.
                        if (!bloomUpdates[descendantUuid]) {
                            bloomUpdates[descendantUuid] = [];
                        }
                        bloomUpdates[descendantUuid].push({
                            resourceUuid: groupUuid,
                            role: groupRoles[groupToInvalidateForResourceUuid]
                        });

                        // C.
                        usersToUpdateForBloom[descendantUuid] = true;
                    }
                });
            }

            // get the complete closure of resource access for the parent group
            _getAllResourceAccess(groupUuid, function(err, access) {
                if (err) {
                    return callback(err);
                }

                // update the bloom for all 'usersToUpdateForBloom' with all this resource access they now have
                for (var resourceUuid in access) {
                    var roles = access[resourceUuid];
                    for (var role in roles) {
                        for (var userToUpdateForBloom in usersToUpdateForBloom) {
                            // aggregate all bloom updates
                            if (!bloomUpdates[userToUpdateForBloom]) {
                                bloomUpdates[userToUpdateForBloom] = [];
                            }
                            bloomUpdates[userToUpdateForBloom].push({
                                resourceUuid: resourceUuid,
                                role: role
                            });
                        }
                ...
                // finally apply the bloom updates, we have them all aggregated
                _bloomUpdateUsersAccess(bloomUpdates, function(err) {
                    if (err) {
                        return callback(err);
                    }
...
/**
 * Given a principal, get all resources to which they have access (directly and indirectly), and all roles they have on those resources.
 *
 * @param {String}      principalUuid   The UUID of the principal to search on
 * @param {Function}    callback        Invoked when the process completes
 * @param {Object}      callback.err    An error that occurred, if any
 * @param {Object}      callback.access A hash keyed by resource UUID whose value (Object) enumerates all roles (as the key) the user has
 *                                      on the resource. @see #getRolesForPrincipalsAndResourceType for an example of the access param
 *                                      format.
 */
var _getAllResourceAccess = function(principalUuid, callback) {
    _getGroupMembershipAncestry(principalUuid, function(err, principalUuids) {
        principalUuids = _.keys(principalUuids);
        principalUuids.push(principalUuid);
        _getRolesForPrincipalsAndResourceType(principalUuids, null, 100000, callback);
    });
}
 ...
 * @param {Boolean}         opts.probablyFalse   A hint specifying if it is unlikely the user has the access. (default: false)
 ...
 */
var isAllowed = module.exports.isAllowed = function(principalUuid, action, resourceUuid, opts, callback) {
    ...
    // only users can support the 'probablyFalse' hint
    if (opts.probablyFalse === true && AuthzUtil.isUserPrincipal(principalUuid)) {
        _bloomGetUserAccess(principalUuid, resourceUuid, action, function(err, hadAccess) {
            if (!err && !hadAccess) {
                // bloom negative is solid. return false
                return callback(null, false);
            } else if (err) {
                log().warn({err: err}, 'Error accessing bloom filter for user %s on resource %s and access %s', principalUuid, resourceUuid, action);
                hadAccess = null;
            }

            // ignore bloom access errors and continue
            return _isAllowed(principalUuid, action, resourceUuid, function(err, isAllowed) {
                if (err) {
                    return callback(err);
                }

                if (!isAllowed && hadAccess === false) {
                    // user used to be allowed to do this, now they can't. fix up the bloom asynchronously
                    update = {};
                    update[principalUuid] = {
                        resourceUuid: resourceUuid,
                        role: action,
                        remove: true
                    };
                    _bloomUpdateUsersAccess(update);
                }
        ...
        return _isAllowed(principalUuid, action, resourceUuid, callback);
    }
}
        ...
        Cassandra.runQuery('SELECT ? FROM AuthzPermissionCache USING CONSISTENCY QUORUM WHERE principalId IN (?)', [permissionCheckKey, principalUuids], function(err, rows) {
            ...
            for (var i = 0, j = rows.length; i < j; i++) {
                ...
                if (col && col.name === permissionCheckKey) {
                    log().debug('Cache hit on %s for %s principals', permissionCheckKey, principalUuids.length);
            ...
            for (var i = 0; i < numRows; i++) {
                ...
                for (var j = 0; j < numCols; j++) {
                    ...
                    if (col.value === _CachePermissionsValue.TRUE) {
                        log().debug('Cache hit on resource %s for %s principals', resourceUuid, principalUuids.length);
                        return callback(null, true);
                    }
                }
            }
...
var _cacheInvalidateForPrincipals = function(principalUuids, callback) {
    ...
    log().debug('Invalidating cache for %s principals', principalUuids.length);

    Cassandra.runQuery('DELETE FROM AuthzPermissionCache USING CONSISTENCY QUORUM WHERE principalId IN (?)', [principalUuids], function(err) {
        if (err) {
            log().error({err: err}, 'Error invalidating permissions cache for %s prinicpals.', principalUuids.length);
        }
        callback(err);
    });
}
...
var _cacheInvalidateResourceForPrincipals = function(principalUuids, resourceUuid, callback) {
    ...
    log().debug('Invalidating cache on %s for %s principals', resourceUuid, principalUuids.length);
    ...
    Cassandra.runQuery('SELECT ?..? FROM AuthzPermissionCache USING CONSISTENCY QUORUM WHERE principalId IN (?)', [start, end, principalUuids], function(err, rows) {
        if (err) {
            log().error({err: err}, 'Error invalidating permission cache for resource %s for %s principals.', resourceUuid, principalUuids.length);
            return callback(err);
        }
        ...
        if (placeholders.length > 0) {
            Cassandra.runQuery('DELETE ' + placeholders.join(',') + ' FROM AuthzPermissionCache USING CONSISTENCY QUORUM WHERE principalId IN (?)', params, function(err) {
                if (err) {
                    log().error({err: err}, 'Error invalidating permission cache for resource %s for %s principals.', resourceUuid, principalUuids.length);
                }
                return callback(err);
 ...
 * @param {Function(err)}   callback        Standard callback function, invoked when the process completes (optional)
 * @param {Object}          callback.err    An error that occurred, if any
 */
var _cacheSetPermissionCheck = function(principalUuid, resourceUuid, action, callback) {
    ...
    log().debug('Setting permission %s for %s in cache.', resourceActionKey, principalUuid);
    Cassandra.runQuery('UPDATE AuthzPermissionCache USING CONSISTENCY QUORUM SET ? = ? WHERE principalId = ?', [resourceActionKey, _CachePermissionsValue.TRUE, principalUuid], function(err) {
        if (err) {
            log().error({err: err}, 'Error setting cache entry for permission %s for %s.', resourceActionKey, principalUuid);
        }
        callback(err);
    });
}
...
/**
 * Assign a batch of updates to the bloom table.
 *
 * @param {Object}      bloomUpdates    The updates object to apply on the bloom
 * @param {Function}    callback        Invoked when the process completes
 * @param {Object}      callback.err    An error that occurred, if any
 */
_bloomUpdateUsersAccess = function(bloomUpdates, callback) {
    callback = callback || function() {};

    var queries = [];
    var total = 0;

    for (var principalUuid in bloomUpdates) {
        var resourceUpdates = bloomUpdates[principalUuid];
        var updatePlaceholders = [];
        var updateParameters = [];
        var deletePlaceholders = [];
        var deleteParameters = [];
        for (var i = 0; i < resourceUpdates.length; i++) {
            total++;
            var resourceUpdate = resourceUpdates[i];
            var resourceKey = resourceUpdate.resourceUuid + ':' + resourceUpdate.role;
            if (!resourceUpdate.remove) {
                updatePlaceholders.push('? = \'1\'');
                updateParameters.push(resourceKey);
            } else {
                deletePlaceholders.push('?');
                deleteParameters.push(resourceKey);
                log().debug('Purging bloom entry for %s with resource key %s', principalUuid, resourceKey);
            }
        }

        if (updateParameters.length > 0) {
            updateParameters.push(principalUuid);
            queries.push({
                query: 'UPDATE AuthzBloomAccess SET ' + updatePlaceholders.join(',') + ' WHERE userId = ?',
                parameters: updateParameters
            });
        }

        if (deleteParameters > 0) {
            deleteParameters.push(principalUuid);
            queries.push({
                query: 'DELETE ' + deletePlaceholders.join(',') + ' FROM AuthzBloomAccess WHERE userId = ?',
    ...
    if (total > 0) {
        Cassandra.runBatchQuery(queries, 'QUORUM', function(err) {
            if (err) {
                log().error({ err: err, bloom: bloomUpdates }, 'Error executing bloom update query to cassandra');
            }
            return callback(err);
        });
    } else {
...
/**
 * Retrieve the bloom value of the user's role on a particular resource.
 * 
 * @param {String}      userUuid            The uuid of the user to search
 * @param {String}      resourceUuid        The uuid of the resource to search
 * @param {String}      role                The role to search
 * @param {Function}    callback            Invoked when the process completes
 * @param {Object}      callback.err        An error that occurred, if any
 * @param {Boolean}     callback.hadAccess  Whether or not the bloom table contains record that this user ever had this record
 */
_bloomGetUserAccess = function(userUuid, resourceUuid, role, callback) {
    var start = Date.now();
    var resourceKey = resourceUuid + ':' + role;
    Cassandra.runQuery('SELECT ? FROM AuthzBloomAccess USING CONSISTENCY QUORUM WHERE userId = ?', [resourceKey, userUuid], function(err, rows) {
        if (err) {
            log().error({err: err}, 'Error access bloom for user %s and resource key %s', userUuid, resourceKey);
            return callback(err);
        }

        Telemetry.append('bloom.time', duration(start));

        var row = rows[0];
        var col = row.get(resourceKey);
        if (col && col.value === '1') {
            Telemetry.incr('bloom.hit.count');
            return callback(null, true);
        } else {
            Telemetry.incr('bloom.miss.count');
            return callback(null, false);
        }
    });
}
...
/**
 * A hash whose values are arrays that need to be flattened into one array.
 *
 * @param   {Object}        hash    The object whose values should be flattened
 * @return  {Array<String>}         The values flattened into one array
 */
var flattenValues = function(hash) {
    if (!hash) {
        return hash;
    }

    var values = {};
    _.values(hash).forEach(function(items) {
