    ...
    if (AuthzUtil.isGroupId(resourceId)) {
        ...
        for (var c = 0; c < changeKeys.length; c++) {
            ...
            if (AuthzUtil.isUserId(changeKeys[c])) {
                ...
                queries.push({
                    'query': 'UPDATE AuthzMembershipsCache SET ? = ? WHERE principalId = ?',
                    'parameters': ['_', 'invalid', changeKeys[c]]
                });
...
var checkGroupMembershipsForUser = function(userId, groupIds, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM AuthzMembershipsCache USING CONSISTENCY QUORUM WHERE principalId = ?', [userId], function(err, rows) {
        if (err) {
            return callback(err);
        }

        // The cache is still valid.
        var cacheStatus = rows[0].get('_');
        if (cacheStatus && cacheStatus.value === 'valid') {
            // Remove the status indicator.
            var memberships = Cassandra.rowToHash(rows[0]);
            delete memberships['_'];
            delete memberships['principalId'];
...
var getPrincipalMemberships = module.exports.getPrincipalMemberships = function(principalId, start, limit, callback) {
    ...
    // we can't use the SELECT FIRST n query as we need to check the `_` column.
    Cassandra.runQuery('SELECT * FROM AuthzMembershipsCache USING CONSISTENCY QUORUM WHERE principalId = ?', [principalId], function (err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        if (cacheStatus && cacheStatus.value === 'valid') {
            ...
            delete hash['principalId'];
            delete hash['_'];
            var all_memberships = _.keys(hash);
            var memberships = [];
            var added = 0;
            for (var i = 0; i < all_memberships.length && added < limit; i++) {
                if (start && all_memberships[i] > start) {
                    memberships.push(all_memberships[i]);
                    added++;
                } else if (!start) {
                    memberships.push(all_memberships[i]);
                    added++;
